...
test('a failed bridge activation schedules an out-of-process rollback restart', () => {
  const upgradeSection = source.slice(
    source.indexOf('async function performManagedRuntimeUpgrade'),
    source.indexOf('// Shared-host upgrade entry point'),
  );
  assert.match(upgradeSection, /Restored previous bridge checkout after upgrade failure[\s\S]{0,900}scheduleManagedRuntimeServiceRestart\('bridge', operationId\)/);
});
...
test('gateway health wait allows slow boots on loaded VPSes (>= 5 minutes)', () => {
  assert.match(source, /waitForGatewayUpgradeHealth\(\{ timeoutMs = 300000 \}/);
});

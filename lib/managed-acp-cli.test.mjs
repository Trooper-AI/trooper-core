...
test('managed ACP recipes are fixed and reject unknown harnesses', () => {
  ...
  assert.equal(claude?.authMode, 'api_key_or_official_host_login');
  assert.match(claude?.authHint || '', /API key.*official host login/i);
  assert.equal(gemini?.binary, 'gemini');
  assert.equal(gemini?.authMode, 'api_key_or_enterprise_credentials');
  assert.match(gemini?.authHint || '', /does not initiate.*Google browser OAuth/i);
  // ACPX bootstraps OpenCode through its fixed npx adapter with a persistent
  // provider config; a global npm package would disappear on container rebuild.
  assert.equal(getManagedAcpCliRecipe('opencode'), null);

...
test('managed ACP recipes are fixed and reject unknown harnesses', () => {
  ...
  const opencode = getManagedAcpCliRecipe('opencode');
  ...
  assert.equal(opencode?.authMode, 'provider_selection_required');
  assert.match(opencode?.authHint || '', /no generic sign-in.*Choose a provider/i);

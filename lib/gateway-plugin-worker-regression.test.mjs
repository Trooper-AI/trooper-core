...
test('startup gateway plugin retries never block bridge HTTP', () => {
  assert.match(bridgeSource, /async function syncBridgeOwnedGatewayPluginsAsync/);
  assert.match(bridgeSource, /void syncBridgeOwnedGatewayPluginsAsync\('startup-retry'\)/);
  assert.doesNotMatch(
    bridgeSource,
    /setTimeout\(\(\) => \{\s*const retryResults = syncBridgeOwnedGatewayPlugins\('startup-retry'\)/,
  );
});

...
const PINNED_RUNTIME_RELEASE = /\/releases\/download\/org-runtime-[a-f0-9]{40}(?:-[a-f0-9]{40})?\/trooper-org-runtime\.tar\.gz(?:[?#].*)?$/i;
const SHA256 = /^[a-f0-9]{64}$/i;
const RUNTIME_ENCRYPTION = new Set(['none', 'aes-256-cbc']);
...
export function validateRuntimeUpgradeRequest({
  scope = 'all',
  target = null,
  runtimeEncryptionPassword = '',
} = {}) {
  ...
  const normalizedTarget = {
    ...
    runtimeTarballPlaintextSha256: String(target.runtimeTarballPlaintextSha256 || '').trim().toLowerCase(),
    runtimeBundleEncryption: String(target.runtimeBundleEncryption || '').trim().toLowerCase(),
  ...
  if (['all', 'bridge'].includes(normalizedScope) && !RUNTIME_ENCRYPTION.has(normalizedTarget.runtimeBundleEncryption)) {
    throw invalidUpgradeRequest(
      'Promoted runtime encryption metadata is missing or unsupported; build a compatible snapshot before updating existing servers',
    );
  }
  const normalizedRuntimeEncryptionPassword = String(runtimeEncryptionPassword || '').trim();
  if (
    ['all', 'bridge'].includes(normalizedScope)
    && normalizedTarget.runtimeBundleEncryption === 'aes-256-cbc'
  ) {
    if (!SHA256.test(normalizedTarget.runtimeTarballPlaintextSha256)) {
      throw invalidUpgradeRequest('Encrypted runtime bundle is missing a verified plaintext sha256');
    }
    if (normalizedRuntimeEncryptionPassword.length < 32) {
      throw invalidUpgradeRequest('Encrypted runtime bundle decryption material is unavailable');
    }
  }
  if (
  ...
  return {
    ...
    runtimeEncryptionPassword: normalizedRuntimeEncryptionPassword,
  };
}

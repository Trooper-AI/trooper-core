 ...
 */
export function firebaseRestAuth(bridgeAuthToken, apiKeysGetter, memberCheck = null) {
  return async (req, res, next) => {
    ...
    // 3. Firebase ID token (+ org membership when a member list is enforced)
    if (token && firebaseApp) {
      const user = await verifyIdToken(token);
      if (user) {
        if (typeof memberCheck === 'function' && !memberCheck(user.uid)) {
          return res.status(403).json({ error: 'not_a_member', message: 'This account is not a member of this workspace.' });
        }

...
test('bridge routes require bridge auth and ACP agent readiness depends on the provider probe', () => {
  ...
  assert.match(source, /opencodeAcpConfigManager\.getAvailability\(\)\.catch\(\(\) => null\)/);
  assert.match(source, /name === 'opencode'[\s\S]{0,220}Boolean\(opencodeProvider\?\.authenticated\)/);

...
- **Draft / rough / quick / preview** — successfully mutate the timeline. Do **not** run brand, perception, frame QA, lint, or full render-to-completion unless the human also asked for production quality or an explicit export.
- **Standard platform deliverable** (TikTok / Reel / Short / “render this”) without production language — mutate the timeline, then \`video_render_start\` and poll \`video_render_status\` until a terminal completed output path exists. Skip brand/perception/frames/lint unless the human asked for production quality.
- **Production / post-ready / publish / final** — full quality bar below.
...
- **HyperFrames failure path (critical):** if motion create/render returns worker unavailable, empty WEBM, or failed status twice, **remove unbaked motion scenes**, keep titles/captions/package, and **finish with video_render_start → complete outputPath**. Do not loop forever on motion. Post-ready without MG beats no export.
- **Zoom packaging:** if render fails with non-monotonic \`inputRange\` / zoom keyframe errors, strip zoom templates and re-render. Prefer a successful draft export over social-punch zooms.
 ...
 * - standard: mutation + completed render (platform delivers) without full visual QA
 * - production: full brand/perception/frames/lint + completed render (explicit quality bar)
 *
 * Explicit packaging requests (captions, zoom, motion graphics, …) still attach
 * on any tier when the user asked for them.
 */
export function inferVideoExecutionContract({ task = '', systemPrompt = '', context = {} } = {}) {
  ...
  const platformDeliverable = /\b(?:tiktoks?|reels?|youtube shorts?|shorts?)\b/.test(taskText);

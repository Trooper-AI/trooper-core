...
const VIDEO_MOTION_CREATE_TOOLS = new Set(['video_motion_scene_create', 'video_motion_scene_update', 'video_motion_place']);
const VIDEO_MOTION_RENDER_TOOLS = new Set(['video_motion_scene_render', 'video_motion_place']);
const VIDEO_MOTION_STATUS_TOOLS = new Set(['video_motion_scene_status', 'video_motion_place']);
const VIDEO_MOTION_PLACE_TOOLS = new Set(['video_motion_place']);
...
- When the request explicitly asks for motion graphics, rank/grade cards, neon scoreboards, layers, animated highlights/callouts, shaders/WebGL, or HTML-in-canvas work, first call \`video_remotion_capabilities\`, then prefer **\`video_motion_place\`** (create+bake+place in one call) with templates like \`grade-card\` / \`scoreboard\` / \`stat-counter\`. Only use create→render→status for custom HTML. Reading a motion skill or creating source alone is not completion.
- **HyperFrames failure path (critical):** if \`video_motion_place\` or motion create/render returns worker unavailable, empty WEBM, degraded, or failed status twice, **remove unbaked motion scenes**, keep titles/captions/package, and **finish with video_render_start → complete outputPath**. Do not loop forever on motion. Editor-ready project + MP4 without MG beats no deliverable.
 ...
 */
/**
 * Skill NAMES are not user intent. Trooper step prompts list the agent's
 * installed skills ("Skills loaded for this step (follow them): Explainer
 * Video, Long Video to Shorts, Multi Clips to Reels, …") — for any org with
 * video skills installed, that line contains video vocabulary on EVERY run.
 * Strip such lines before classification, or a Notion-app coding step gets
 * bulldozed by the video continuation checklist (this happened).
 */
function stripSkillCatalogLines(text = '') {
  return String(text || '')
    .split('\n')
    .filter((line) => !/^\s*(?:\*\*)?skills loaded for this step\b/i.test(line)
      && !/^\s*\[installed skills:/i.test(line)
      && !/^\s*installed skills(?: available)?\b/i.test(line))
    .join('\n');
}

export function inferVideoExecutionContract({ task = '', systemPrompt = '', context = {} } = {}) {
  // Explicit caller veto — Trooper knows when a run is NOT video work
  // (e.g. a non-media task-runner step). Nothing below may override it.
  if (context?.videoExecution === false) {
    return {
      enabled: false,
      tier: 'none',
      maxContinuationAttempts: 0,
      requiresMutation: false,
      requiresRenderStart: false,
      requiresRenderStatus: false,
    };
  }
  const taskText = stripSkillCatalogLines(String(task || '')).toLowerCase();
  // Classification reads the TASK text only — deliberately NOT the system
  // prompt (it always embeds the installed-skills catalog) and NOT a JSON
  // dump of context (it carries the installedSkills payload). Both channels
  // permanently contain video vocabulary for orgs with video skills, which
  // is exactly how non-video runs got misclassified. `systemPrompt` stays in
  // the signature for call-site compatibility.
  void systemPrompt;
  // The media execution lane carries IMAGE and MUSIC generation too — the
  // lane alone must never imply video, or an image-only run (e.g. an article
  // with generated illustrations) gets bulldozed by the video continuation
  // checklist mid-run. Lane counts only alongside actual video vocabulary.
  const videoVocabulary = /\b(?:videos?|clips?|footage|mp4|mov\b|webm|reels?|tiktoks?|youtube shorts?|podcast|talking[- ]head|b[- ]roll|long[- ]to[- ]shorts?)\b/.test(taskText);
  const videoSignal = (context?.executionLane === 'media' && videoVocabulary)
    ...
    || /\bvideo_(?:project|edit|timeline|cut|make_shorts|render|transcript|highlights|perception)\b/.test(taskText)
    || /\b(?:tiktoks?|reels?|youtube shorts?|video edit|podcast clips?|long[- ]to[- ]shorts?)\b/.test(taskText);
...
export function evaluateVideoExecutionCompletion(toolLog = [], contract = {}) {
  ...
  const motionPlaceComplete = successfulEntries.some((entry) => {
    if (!VIDEO_MOTION_PLACE_TOOLS.has(String(entry.tool || '').trim().toLowerCase())) return false;
    const payload = (entry.result && typeof entry.result === 'object')
      ? entry.result
      : safeJsonParse(entry.summary);
    if (!payload || payload.ok === false || payload.placed === false) return false;
    // Place is complete when bake finished or job reports complete.
    const job = payload.job || {};
    const status = String(job.status || payload.status || '').toLowerCase();
    if (payload.placed === true || payload.ok === true) {
      if (['failed', 'error', 'cancelled'].includes(status)) return false;
      return true;
    }
    return ['complete', 'completed', 'succeeded', 'ready'].includes(status);
  });
  const motionSceneCreate = hasAny(VIDEO_MOTION_CREATE_TOOLS, tools) || motionPlaceComplete;
  const motionSceneRender = hasAny(VIDEO_MOTION_RENDER_TOOLS, tools) || motionPlaceComplete;
  const motionSceneStatus = hasAny(VIDEO_MOTION_STATUS_TOOLS, tools) || motionPlaceComplete;
  ...
  const motionSceneComplete = motionPlaceComplete || successfulEntries.some((entry) => {
...
5) Motion if required: video_motion_place (grade-card/scoreboard/…) — not create-only
6) If motion was required and failed twice: remove unbaked scenes and continue
7) Production QA only if required: video_frames + lint
8) video_render_start → poll until completed outputPath

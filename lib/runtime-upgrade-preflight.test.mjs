...
test('runtime upgrade preflight fails closed for nearly-full disk and unavailable Docker', () => {
  // Floor is intentionally low (256 MiB for scope=all) so CX23-class boxes with
  // a few GB free can upgrade. Only fail when the volume is essentially full.
  const result = runRuntimeUpgradePreflight({
    scope: 'all',
    dependencies: dependencies({ diskMb: 64, dockerHealthy: false }),
  });
  ...
  assert.ok(result.checks.some((check) => check.name === 'docker-daemon' && !check.ok));
  assert.match(
    result.checks.find((check) => check.name === 'disk')?.detail || '',
    /need at least \d+ MiB free/i,
  );
  assert.throws(
    () => assertRuntimeUpgradePreflight({
      scope: 'all',
...
test('runtime upgrade preflight allows multi-GB-tight CX23 disks (uses full free space)', () => {
  // The old 4 GiB reserve blocked healthy CX23 orgs with ~2.6 GiB free.
  const result = runRuntimeUpgradePreflight({
    scope: 'all',
    dependencies: dependencies({ diskMb: 2641 }),
  });
  assert.equal(result.ok, true);
  assert.ok(result.checks.some((check) => check.name === 'disk' && check.ok));
  assert.equal(result.requiredDiskMb, 256);
});

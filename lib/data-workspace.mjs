...
export function registerDataApiRoutes(app) {
  ...
  // GET /api/data/objects/:name/routines — find cron jobs that reference this table.
  // Scans cron job messages for the table name. Simple convention-based linking.
  app.get('/api/data/objects/:name/routines', async (req, res) => {
    try {
      const tableName = req.params.name;
      const { readFile } = await import('fs/promises');
      const cronPath = openclawConfigPath('cron', 'jobs.json');
      const raw = await readFile(cronPath, 'utf8').catch(() => null);
      if (!raw) return res.json({ routines: [] });

      const data = JSON.parse(raw);
      const jobs = data.jobs || [];

      // Find jobs whose message mentions this table name
      const linked = jobs.filter(job => {
        ...
        return msg.includes(name) || msg.includes(name.replace(/_/g, ' '));
      });

      res.json({ routines: linked, table: tableName });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  });

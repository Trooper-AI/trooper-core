...
test('an assistant closer offering next steps is NOT an approval prompt — no numbered-yes line, no match', () => {
  // False positives here notify the user AND hold finalization for 30 minutes,
  // so the claude rule demands the dialog's numbered option list.
  assert.equal(
    classifyAcpObservation('claude', { text: 'Done. Do you want me to proceed with deploying as well?' }),
    null,
  );
});

test('codex prose mentioning "allow this command" is not a live prompt — the matcher is line-anchored', () => {
  assert.equal(
    classifyAcpObservation('codex', { text: 'To fix this, you should allow this command in your sandbox settings.' }),
    null,
  );
  assert.equal(
    classifyAcpObservation('codex', { text: 'Allow command? [y/N]' })?.state,
    'awaiting_user',
  );
});

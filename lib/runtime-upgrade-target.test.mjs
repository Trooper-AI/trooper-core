...
const target = {
  ...
  runtimeTarballPlaintextSha256: 'd'.repeat(64),
  runtimeBundleEncryption: 'aes-256-cbc',
...
test('validateRuntimeUpgradeRequest accepts a fully immutable target', () => {
  assert.deepEqual(validateRuntimeUpgradeRequest({
    scope: 'all',
    target,
    runtimeEncryptionPassword: 'p'.repeat(32),
  }), {
    scope: 'all',
    target,
    runtimeEncryptionPassword: 'p'.repeat(32),
...
test('validateRuntimeUpgradeRequest rejects floating or incomplete targets', () => {
  ...
  assert.throws(
    () => validateRuntimeUpgradeRequest({
      scope: 'bridge',
      target: { ...target, runtimeTarballPlaintextSha256: null },
      runtimeEncryptionPassword: 'p'.repeat(32),
    }),
    /plaintext sha256/,
  );
  assert.throws(
    () => validateRuntimeUpgradeRequest({
      scope: 'bridge',
      target: { ...target, runtimeBundleEncryption: null },
      runtimeEncryptionPassword: 'p'.repeat(32),
    }),
    /encryption metadata/,
  );
...
test('immutable runtime bundle validation supports release assets and commit tags', () => {
  ...
  assert.equal(
    isImmutableRuntimeBundleUrl(
      `https://github.com/example/runtime/releases/download/org-runtime-${'c'.repeat(40)}-${'d'.repeat(40)}/trooper-org-runtime.tar.gz`,
    ),
    true,
  );
  assert.equal(
...
test('upgrade routes and updater do not use floating production artifacts', () => {
  ...
  const bridgeUpdaterSource = readFileSync(new URL('../scripts/update-bridge.sh', import.meta.url), 'utf8');
  const updateCheckSource = readFileSync(new URL('../scripts/check-update.sh', import.meta.url), 'utf8');
  const versionInfoSource = readFileSync(new URL('./version-info.mjs', import.meta.url), 'utf8');
  const apiRoutesSource = readFileSync(new URL('./api-routes.mjs', import.meta.url), 'utf8');
  ...
  assert.ok(
    managedUpgradeSection.indexOf('assertRuntimeUpgradePreflight')
      < managedUpgradeSection.indexOf("runUpgradeCommand('bash', ['/opt/openclaw-bridge/scripts/update-bridge.sh'"),
  );
  assert.match(managedUpgradeSection, /update-bridge\.sh', 'activate'/);
  assert.match(managedUpgradeSection, /update-bridge\.sh', 'rollback'/);
  assert.match(managedUpgradeSection, /safe\.directory=\/opt\/openclaw-bridge/);
  assert.doesNotMatch(managedUpgradeSection, /reset',\s*'--hard'/);
  assert.doesNotMatch(managedUpgradeSection, /cwd:\s*'\/opt\/openclaw-bridge'[\s\S]{0,120}npm/);
  ...
  assert.match(bridgeUpdaterSource, /NEXT_DIR="\$\{INSTALL_DIR\}\.next"/);
  assert.match(bridgeUpdaterSource, /PREVIOUS_DIR="\$\{INSTALL_DIR\}\.previous"/);
  assert.match(bridgeUpdaterSource, /git -c safe\.directory="\$INSTALL_DIR" -C "\$INSTALL_DIR" rev-parse HEAD/);
  assert.match(bridgeUpdaterSource, /git -C "\$NEXT_DIR" fetch -q --depth 1 origin "\$TARGET_COMMIT"/);
  assert.match(bridgeUpdaterSource, /AFTER_SHA=.*git -C "\$NEXT_DIR" rev-parse HEAD/);
  assert.ok(
    bridgeUpdaterSource.indexOf('AFTER_SHA=')
      < bridgeUpdaterSource.indexOf('Activating staged bridge'),
  );
  assert.match(bridgeUpdaterSource, /cd "\$NEXT_DIR" && npm ci --omit=dev/);
  assert.match(bridgeUpdaterSource, /copy_mutable_state "\$INSTALL_DIR" "\$NEXT_DIR"/);
  assert.match(bridgeUpdaterSource, /device-identity\.json/);
  assert.match(bridgeUpdaterSource, /agent-registry\.json/);
  assert.match(bridgeUpdaterSource, /rollback_bridge/);
  ...
  assert.match(updateCheckSource, /git -c safe\.directory=\/opt\/openclaw-bridge/);
  assert.match(versionInfoSource, /git -c safe\.directory=\/opt\/openclaw-bridge/);
  assert.match(apiRoutesSource, /git -c safe\.directory=\/opt\/openclaw-bridge/);

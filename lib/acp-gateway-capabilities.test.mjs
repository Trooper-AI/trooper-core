...
test('no ACP is exposed until a canary harness is explicitly configured', () => {
  const result = buildAcpGatewayCapabilities({ canaryHarnesses: '' });
  assert.equal(result.integrations.some((entry) => entry.exposed), false);
  assert.equal(result.integrations.some((entry) => entry.canaryValidated), false);
});

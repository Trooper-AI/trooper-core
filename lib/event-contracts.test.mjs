...
test('normalizeBridgeEventPayload adds versioned run/session envelope fields without losing existing keys', () => {
  const payload = normalizeBridgeEventPayload('tool_start', {
    ...
    sessionKey: 'agent:main:hook:trooper:test:chat',
    runId: 'run-1',
    source: 'live_stream',
    sequence: 7,
    time: 123,
  ...
  assert.equal(payload.sessionKey, 'agent:main:hook:trooper:test:chat');
...
test('tool result contract preserves provider-reported media usage and actual cost metadata', () => {
  const payload = normalizeToolEventPayload('tool_result', {
    tool: 'image_generate',
    toolCallId: 'image-call-1',
    params: { quality: 'low', resolution: '1K' },
    result: {
      media: [{ url: 'https://media.example.test/cat.jpg', contentType: 'image/jpeg' }],
      count: 1,
    },
    details: {
      provider: 'openai',
      model: 'gpt-image-1',
      usage: { cost_usd: 0.04, images: 1 },
      normalization: { applied: { resolution: '1K' } },
    },
    success: true,
  });

  assert.equal(payload.toolCallId, 'image-call-1');
  assert.equal(payload.provider, 'openai');
  assert.equal(payload.model, 'gpt-image-1');
  assert.deepEqual(payload.result.media, [{ url: 'https://media.example.test/cat.jpg', contentType: 'image/jpeg' }]);
  assert.deepEqual(payload.media, [{ url: 'https://media.example.test/cat.jpg', contentType: 'image/jpeg' }]);
  assert.deepEqual(payload.details.usage, { cost_usd: 0.04, images: 1 });
  assert.deepEqual(payload.usage, { cost_usd: 0.04, images: 1 });
  assert.equal(payload.costUsd, 0.04);
  assert.equal(payload.costSource, 'provider_reported');
  assert.equal(payload.estimated, false);
  assert.equal(payload.pricingStatus, 'actual');
});

test('tool result contract marks media pricing unavailable without inventing a charge', () => {
  const payload = normalizeToolEventPayload('tool_result', {
    tool: 'image_generate',
    toolCallId: 'image-call-2',
    result: { media: [{ path: '/home/node/.openclaw/media/cat.jpg' }] },
    details: { provider: 'replicate', model: 'black-forest-labs/flux-schnell' },
    success: true,
  });

  assert.equal(payload.provider, 'replicate');
  assert.equal(payload.model, 'black-forest-labs/flux-schnell');
  assert.equal(payload.costUsd, undefined);
  assert.equal(payload.costSource, 'unavailable');
  assert.equal(payload.estimated, false);
  assert.equal(payload.pricingStatus, 'unavailable');
});

test('forwarded tool result retains tool identity, media result, and explicit estimate semantics', () => {
  const payload = forwardToolEventPayload('tool_result', {
    tool: 'image_generate',
    toolCallId: 'image-call-3',
    params: { count: 2 },
    result: { media: [{ path: '/tmp/a.jpg' }, { path: '/tmp/b.jpg' }], count: 2 },
    provider: 'openrouter',
  ...
  assert.equal(payload.toolCallId, 'image-call-3');
  assert.equal(payload.provider, 'openrouter');
  assert.equal(payload.model, 'google/gemini-2.5-flash-image');
  assert.equal(payload.media.length, 2);
  assert.equal(payload.costUsd, 0.08);
  assert.equal(payload.costSource, 'service_price_estimate');
  assert.equal(payload.estimated, true);
  assert.equal(payload.pricingStatus, 'estimated');
});
...
test('buildScreenshotFramePayload keeps optional viewport metadata when provided', () => {
  assert.deepEqual(buildScreenshotFramePayload({
    ...
    geometry: '1920x1080',
    screenshotPath: '/home/node/.openclaw/media/browser/screenshot-2026-05-28.png',
  }), {
    base64: 'abcd',
    timestamp: 123,
    action: 'Visible browser viewport',
...
test('extractHistoryToolEvents normalizes assistant tool calls and tool results into a stable contract', () => {
  ...
  assert.equal(events[0].data.sessionKey, 'agent:main:hook:trooper:test:chat');
...
test('extractHistoryToolEvents carries image-generation requested and normalized settings', () => {
  const events = extractHistoryToolEvents([
    {
  ...
  assert.deepEqual(events[0].data.imageGeneration.requested, {
    size: '1024x1024',
    resolution: '1K',
    quality: 'low',
    outputFormat: 'jpeg',
  });
  assert.deepEqual(events[1].data.imageGeneration.applied, { resolution: '1K' });
  assert.deepEqual(events[1].data.imageGeneration.ignoredOverrides, ['outputFormat']);
});

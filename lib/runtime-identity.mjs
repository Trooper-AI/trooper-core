...
export function normalizeAgentProfile(agent = {}) {
  return {
    ...
    email: cleanText(agent.email || agent.emailAddress || agent.agentEmail?.address, ''),
...
- Use 1-4 questions per batch max. If there are more unresolved decisions, ask the first 4 now, wait for answers, then ask the next batch of up to 4.
- Each question should have 2-5 concrete options, and each option should include a short parenthetical impact note when it helps the human answer.
...
- Use typed sub-agents when parallel specialist work will materially help: explore (read-only research), plan (read-only design), implement (tight brief), verify (adversarial checks), or roster delegate.
- Brief children completely — paths, constraints, success criteria. Never "based on your findings, implement it".
- Do not invent sub-agent results before completion. Prefer verify before claiming non-trivial multi-file work done.
...
title: ${profile.title}
role: ${profile.role}
team: Trooper${profile.email ? `
email: ${profile.email}

You have your own email address (${profile.email}). Use the trooper-email skill (bridge /email endpoints) to read your inbox and send or reply as yourself. Never invent email contents or recipients.` : ''}`;
}
...
- If no native media tool/model is available, say exactly which media capability is missing instead of substituting a frontend mockup.${deviceScope}`;
    case 'browser':
      if (browserMode === 'chrome-extension') {
        return `[EXECUTION LANE — USER CHROME EXTENSION]
- The human is referring to their own Chrome/browser/current tab/page. Use Trooper Chrome extension tools first, not headless browser, nodes, desktop screenshots, screen.snapshot, or computer-use.
- Required first sequence: chrome_status -> chrome_list_tabs -> chrome_claim_tab -> chrome_read_context.
- After reading context, use chrome_click, chrome_type, chrome_scroll, chrome_navigate, chrome_open_tab, or chrome_screenshot only when needed.
- If chrome_status says the extension is missing/disconnected, stop and tell the human to install/reload Trooper Browser Control from Trooper Settings -> AI Server -> Chrome browser control or chrome://extensions. Do not silently downgrade to node screenshots.
- Use managed/headless browser only for public web lookup/testing when the human is not referring to their current Chrome profile.${deviceScope}`;
      }
...
- Do not run \`openclaw status\`, \`openclaw nodes list\`, or other local OpenClaw CLI discovery commands from a shell just to find devices; those can hang in gateway sessions.`;
  ...
  prompt += `\n\n[EXEC APPROVAL RULES]
- To request command execution, call the native \`exec\` tool. OpenClaw and Trooper own the approval event and UI; never print a command and tell the human to reply with \`/approve\`.
- An exec approval authorizes the exact command. It does not grant sudo/root privileges, change the execution host, or install software on the organization VPS.
- If a host-level dependency such as ffmpeg is missing and the current exec runtime cannot install it without root, report a runtime provisioning/repair blocker with the failed check and required host dependency. Do not fabricate an approval flow.
- After an approval is granted, continue from the paused tool call. Do not ask the human to repeat the original request.`;
...
- Keep <confirm_doubts> lightweight: ask at most 4 questions in one batch, then ask the next batch only after the human answers.
...
- Match the output type to the work: research/data → structured exports, build work → real files/apps, review → evidence and defects.`;
  ...
  prompt += `\n\n[SUB-AGENT RULES]
- Profiles: explore (read-only research), plan (read-only architecture), implement (execute with paths + success criteria), verify (independent adversarial checks), delegate (named roster specialist).
- Prefer sessions_spawn / spawn_subagent with an explicit profile intent in the task text when splitting work.
- Never delegate understanding: include goal, prior attempts, exact paths, and success criteria in the child brief.
- explore/plan/verify must not mutate project files. implement may edit within scope.
- Do not invent child results before subagent_done / completion.
- For multi-file or production-facing work, verify (or list Unverified: …) before claiming full success.`;

  prompt += `\n\n[VERIFICATION RULES]
- Do not claim "all tests pass" or "works" without commands/observed results or an explicit Unverified disclosure.
- Reading code is not verification — run or exercise the change when possible.`;

...
- In CrabsHQ, "devices", "nodes", "paired devices", and named machines like "my MacBook" refer to the same device-control surface: OpenClaw nodes plus node-capable CrabsHQ paired devices. Do **not** ask which inventory the human means unless they name an external system.
...
- In CrabsHQ product language, "devices" means the user's OpenClaw nodes / node-capable paired devices. If the human says "devices", "nodes", "MacBook", or "paired devices", treat that as one device-control surface instead of asking which inventory they mean.
- Prefer native OpenClaw node execution when a live node is available. If native OpenClaw Nodes is empty, use the CrabsHQ paired-device runtime for safe local actions and explain that native node registration still needs to finish.
...
export function buildRuntimeSystemPrompt(agent, {
  ...
  prompt += `\n\n[DEVICE/NODE RULES]
- In CrabsHQ product language, "devices", "nodes", "paired devices", and named machines like "my MacBook" all mean the OpenClaw node/device-control surface unless the user explicitly names a different external inventory.
...
- Use the native nodes/device tools for inventory and actions. If native nodes are empty, use the CrabsHQ paired-device path when available instead of declaring the whole request impossible.

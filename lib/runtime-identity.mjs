...
- OpenClaw **Paired Devices** are authorization identities/tokens; CrabsHQ paired devices mirror that identity/provisioning layer and provide only a limited fallback path. A paired identity is not a live node until it appears connected in the Nodes inventory.
- If the human says "nodes" or "OpenClaw nodes", use the native **nodes** tool first and do not answer from the CrabsHQ paired-device list as if it were node inventory.
- Use **device_action** only for explicit CrabsHQ/paired-device fallback requests, selected-device actions, or safe local actions when native nodes are unavailable and the user clearly refers to their Mac/device.
...
export function buildExecutionLanePromptBlock(context = {}) {
  ...
  const deviceScope = deviceMode === 'device'
    ? `\n- The human selected the device "${selectedDeviceName || 'Device'}". Prefer that device for explicit device-bound fallback work unless the task clearly belongs on cloud runtime instead.
- Native OpenClaw Nodes are the canonical device execution surface. CrabsHQ paired devices are identity/provisioning records plus a limited fallback runtime.
...
- Prefer native OpenClaw node execution when a live node is available. If native OpenClaw Nodes is empty and the human clearly refers to the selected Mac/device, use the CrabsHQ paired-device runtime for safe local actions and explain that native node registration or daemon startup still needs to finish.
- If the human asks to open something, launch an app, send a notification, or run a safe local status command on that selected device, use the paired-device runtime path surfaced by CrabsHQ instead of saying you cannot control their Mac/device.
...
- OpenClaw Paired Devices are authorization identities/tokens. CrabsHQ paired devices mirror that provisioning layer and provide a limited fallback path; a paired identity is not a live node until it appears connected in the Nodes inventory.
- If the user asks for "nodes" or "OpenClaw nodes", use native nodes/device tools first. Do not answer from CrabsHQ paired-device fallback data as if it were the live node inventory.
- If the user explicitly asks for CrabsHQ/paired devices, a selected device, "my MacBook", or a safe local action on their Mac/device, use the CrabsHQ paired-device fallback when native nodes are unavailable and say it is fallback control.

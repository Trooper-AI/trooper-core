...
- Prefer native OpenClaw node execution when a live node is available. If native OpenClaw Nodes is empty and the human clearly refers to the selected Mac/Windows PC/personal computer, use the CrabsHQ paired-device runtime for safe local actions and explain that native node registration or daemon startup still needs to finish.
...
- If the user explicitly asks for CrabsHQ/paired devices, a selected personal computer, "my MacBook", "my Windows", "my PC", "my personal computer", or a safe local action on their Mac/Windows/personal device, use the CrabsHQ paired-device fallback when native nodes are unavailable and say it is fallback control.

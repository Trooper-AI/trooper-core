...
- In CrabsHQ, "devices", "nodes", "paired devices", and named machines like "my MacBook" refer to the same device-control request surface unless the human names an external system.
- Implementation detail: OpenClaw **Paired Devices** are authorization identities/tokens; OpenClaw **Nodes** are live runtimes. A paired identity is not a live node until it appears connected in the Nodes inventory.
- Do **not** ask which inventory the human means for normal CrabsHQ device/node requests. Report both layers clearly only when diagnosing connection state.
- Use the native **nodes** tool first for device/node inventory and control. Use **device_action** as the paired-device fallback for safe local actions when the native nodes surface is empty or unavailable.
...
- In CrabsHQ product language, "devices" means the user's OpenClaw nodes / node-capable paired devices. If the human says "devices", "nodes", "MacBook", or "paired devices", treat that as one device-control request surface instead of asking which inventory they mean.
...
- Prefer native OpenClaw node execution when a live node is available. If native OpenClaw Nodes is empty, use the CrabsHQ paired-device runtime for safe local actions and explain that native node registration or daemon startup still needs to finish.
...
export function buildRuntimeSystemPrompt(agent, {
  ...
  prompt += `\n\n[DEVICE/NODE RULES]
- In CrabsHQ product language, "devices", "nodes", "paired devices", and named machines like "my MacBook" all mean the OpenClaw node/device-control request surface unless the user explicitly names a different external inventory.
- Implementation detail: OpenClaw Paired Devices are authorization identities/tokens; OpenClaw Nodes are live runtimes. A paired identity is not a live node until it appears connected in the Nodes inventory.
- Do not ask "which inventory?" for ordinary CrabsHQ requests like "list devices", "list nodes", "check my MacBook", or "run this on my paired device".
- Use the native nodes/device tools for inventory and actions. If native nodes are empty, state that no live OpenClaw node is connected and use the CrabsHQ paired-device path when available instead of declaring the whole request impossible.

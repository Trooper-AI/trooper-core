...
export function detectPairedDeviceIntent(message, { deviceRef = null } = {}) {
  ...
  const wantsInventory = /\b(list|show|what|which)\b/.test(normalized)
    && (
...
export function formatRuntimeDeviceList({ devices = [], pendingApprovals = [] } = {}) {
  ...
  lines.push('');
  lines.push('In CrabsHQ, user-facing "devices" should be treated as the node/device inventory first. If native OpenClaw Nodes is empty, use these paired-device runtimes for safe local actions and mention that native node registration still needs to finish.');
  return lines.join('\n');
}
...
export function formatDeviceHealthCheck(device, commandResults = []) {
  ...
  lines.push('');
  lines.push('This used the CrabsHQ paired-device fallback path inside the same devices/nodes control surface.');
  return lines.join('\n');
}

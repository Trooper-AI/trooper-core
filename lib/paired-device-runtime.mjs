...
export function detectPairedDeviceIntent(message, { deviceRef = null } = {}) {
  ...
  const explicitPairedFallback = Boolean(
    ...
    || normalized.includes('Trooper device')
    || normalized.includes('Trooper devices')
  ...
  const shortInventoryClarification = /^(yes\s+)?(i\s+mean\s+)?(?:paired\s+devices?|Trooper\s+devices?)$/i.test(normalized);
  if (wantsInventory || shortInventoryClarification) {
    // Inventory must come from OpenClaw's native devices/nodes layer. The
    // Trooper fallback runtime can run selected safe actions, but it should not
...
export function formatRuntimeDeviceList({ devices = [], pendingApprovals = [] } = {}) {
  if (!Array.isArray(devices) || devices.length === 0) {
    return 'I could not find any Trooper paired-device fallback identities right now.\n\nNative OpenClaw Nodes are still the canonical execution inventory; this fallback list is only Trooper authorization/provisioning metadata.';
  }

  const lines = [`I found ${devices.length} Trooper paired-device fallback target${devices.length === 1 ? '' : 's'}:`];
  ...
  lines.push('');
  lines.push('Native OpenClaw Nodes are the canonical execution targets. Trooper paired devices are authorization/provisioning identities plus a limited fallback path for safe local actions; do not treat one as a live node unless the native OpenClaw Nodes inventory shows it connected.');
  return lines.join('\n');
}
...
export function formatDeviceHealthCheck(device, commandResults = []) {
  ...
  lines.push('');
  lines.push('This used the Trooper paired-device fallback path. It is authorized device control, not proof that the native OpenClaw node daemon is live.');
  return lines.join('\n');
}

...
function sanitizeAcpDisplayText(value = '') {
  ...
  if (looksLikeFailureText(text)) {
    ...
    return (errorLine || text).slice(0, 280);
  }
  if (looksLikeSystemPromptDump(text)) {
  ...
  if (text.length > 1200) return `${text.slice(0, 1199).trim()}…`;
  return text;
}
    ...
    for (const [partIndex, part] of parts.entries()) {
      if (part.type === 'text') {
        ...
        const event = {
          ...
          partIndex,
          createdAt,
          ...(failed ? { errorKind: /\busage limit|UsageLimitExceeded|Quota exceeded\b/i.test(part.text) ? 'acp_quota_exhausted' : 'acp_turn_failed' } : {}),
        };
        events.push(event);

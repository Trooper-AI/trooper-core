...
test('idle does not finish while steer is pending (avoids premature Done)', async () => {
  const { scheduler, registry, local, state } = createHarness();
  local._steerPending = true;
  let final = null;
  ...
  local.status = 'failed';
  local.output = 'UsageLimitExceeded';
  await scheduler.advance(ACP_POLL_ACTIVE_MS);
  assert.equal(final?.type, 'error');
  assert.equal(final?.status, 'failed');
});

test('idle with UsageLimitExceeded in output finishes as failed', async () => {
  const { scheduler, registry, local, state } = createHarness();
  local._steerPending = false;
  local.output = "You've hit your usage limit. Some(UsageLimitExceeded)";
  let final = null;
  registry.subscribe('s1', local, { onTerminal: (f) => { final = f; } });
  state.status = 'idle';
  await scheduler.advance(1);
...
test('idle with no assistant output eventually fails instead of empty success', async () => {
  const { scheduler, registry, local, state } = createHarness();
  local._steerPending = false;
  let final = null;
  registry.subscribe('s1', local, { onTerminal: (f) => { final = f; } });
  state.status = 'idle';
  await scheduler.advance(1);
  assert.equal(final, null, 'should keep watching briefly for first token');
  // Idle cadence is 2s; overshoot the 120s soft-wait so the fail tick runs.
  await scheduler.advance(130_000);
  assert.equal(final?.type, 'error');
  assert.equal(final?.status, 'failed');
  assert.match(String(local.output || ''), /without returning any output/i);
});

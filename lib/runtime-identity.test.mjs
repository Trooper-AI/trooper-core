...
test('device mode treats CrabsHQ devices and nodes as one device-control surface', () => {
  const prompt = buildExecutionLanePromptBlock({
    executionLane: 'browser',
    deviceRef: {
      mode: 'device',
  ...
  assert.match(prompt, /devices.*OpenClaw nodes/i);
  ...
  assert.match(prompt, /instead of asking which inventory/i);
...
test('specialist mode and runtime prompt reinforce data-first non-markdown behavior', () => {
  ...
  assert.match(runtimePrompt, /devices.*nodes.*paired devices/i);
  assert.match(runtimePrompt, /Paired Devices are authorization identities/i);
  assert.match(runtimePrompt, /Nodes are live runtimes/i);
  assert.match(runtimePrompt, /Do not ask "which inventory\?"/i);

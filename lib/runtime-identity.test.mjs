...
test('chrome-extension browser mode requires extension tools before screen/node fallback', () => {
  const prompt = buildExecutionLanePromptBlock({
    executionLane: 'browser',
    browserTask: true,
  ...
  assert.match(prompt, /USER CHROME EXTENSION/);
  assert.match(prompt, /chrome_status -> chrome_list_tabs -> chrome_claim_tab -> chrome_read_context/);
  assert.match(prompt, /not headless browser, nodes, desktop screenshots, screen\.snapshot, or computer-use/i);
  assert.match(prompt, /Do not silently downgrade to node screenshots/i);
});
...
test('runtime prompt includes media tool override rule', () => {
  ...
  assert.match(runtimePrompt, /call the native `exec` tool/i);
  assert.match(runtimePrompt, /never print a command and tell the human to reply with `\/approve`/i);
  assert.match(runtimePrompt, /does not grant sudo\/root privileges/i);
  assert.match(runtimePrompt, /runtime provisioning\/repair blocker/i);
});
...
test('runtime prompt tells agents to ask long-work doubts with confirm_doubts', () => {
  ...
  assert.match(runtimePrompt, /at most 4 questions in one batch/i);
...
test('workspace identity files include mindful confirm_doubts question contract', () => {
  ...
  for (const agentsMd of [leadFiles['AGENTS.md'], spcFiles['AGENTS.md']]) {
    ...
    assert.match(agentsMd, /1-4 questions per batch max/i);
    assert.match(agentsMd, /next batch of up to 4/i);
...
test('runtime prompt includes typed sub-agent and verification rules', () => {
  const runtimePrompt = buildRuntimeSystemPrompt({ name: 'Jordan', title: 'Chief of Staff', role: 'LEAD' }, {
    channel: 'general',
  });
  assert.match(runtimePrompt, /\[SUB-AGENT RULES\]/);
  assert.match(runtimePrompt, /explore \(read-only research\)/);
  assert.match(runtimePrompt, /Never delegate understanding/i);
  assert.match(runtimePrompt, /\[VERIFICATION RULES\]/);
  assert.match(runtimePrompt, /Unverified/i);
});

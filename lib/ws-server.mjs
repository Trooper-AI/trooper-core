...
export class BridgeWSServer {
  constructor({ server, path = '/ws', bridgeAuthToken = '', memberCheck = null }) {
    ...
    // Org-membership authorization for direct browser clients (uid → bool).
    // Fail-closed: when set, a verified Firebase identity is not enough.
    this.memberCheck = typeof memberCheck === 'function' ? memberCheck : null;
    this._setupHandlers();
  ...
  async _handleMessage(ws, msg) {
    ...
    switch (msg.type) {
      case 'identify': {
        ...
        if (!client.authenticated && (isAuthEnabled() || this.bridgeAuthToken)) {
          // Secured deployment (managed VPSs always set a bridge token):
          // browser clients MUST present a verifiable Firebase identity. The
          // old fallthrough accepted any non-matching token as anonymous.
          if (!isAuthEnabled()) {
            ws.send(JSON.stringify({ type: 'error', message: 'Direct sign-in is not enabled on this server yet.' }));
            return ws.close(4003, 'Firebase auth not configured');
          }
          ...
          if (this.memberCheck && !this.memberCheck(user.uid)) {
            ws.send(JSON.stringify({ type: 'error', message: 'Not a member of this workspace' }));
            return ws.close(4004, 'not_a_member');
          }
          client.user = user;
          client.authenticated = true;
          ...
          // Genuine dev mode — neither a bridge token nor Firebase configured.

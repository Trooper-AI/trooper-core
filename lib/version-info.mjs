...
export function readBridgeVersion({ ttlMs = 30000, force = false } = {}) {
  ...
  const value = {
    ...
    bridgeSha: safeExec('git -c safe.directory=/opt/openclaw-bridge -C /opt/openclaw-bridge rev-parse --short HEAD 2>/dev/null'),
    bridgeFullSha: safeExec('git -c safe.directory=/opt/openclaw-bridge -C /opt/openclaw-bridge rev-parse HEAD 2>/dev/null'),
    bridgeCommittedAt: safeExec('git -c safe.directory=/opt/openclaw-bridge -C /opt/openclaw-bridge log -1 --format=%cI 2>/dev/null'),
    bridgeBranch: safeExec('git -c safe.directory=/opt/openclaw-bridge -C /opt/openclaw-bridge rev-parse --abbrev-ref HEAD 2>/dev/null'),
    bridgeDirty: Boolean(safeExec('git -c safe.directory=/opt/openclaw-bridge -C /opt/openclaw-bridge status --porcelain 2>/dev/null')),

...
test('video_motion_place success satisfies motion create/render/status', () => {
  const contract = inferVideoExecutionContract({
    task: 'Create a production TikTok with motion graphics scoreboard overlay.',
    context: { executionLane: 'media' },
  });
  const result = evaluateVideoExecutionCompletion([
    { tool: 'video_remotion_capabilities', success: true },
    ...
    { tool: 'video_package_short', success: true, params: { title: 'Hook' } },
    {
      tool: 'video_motion_place',
      success: true,
      result: {
        ok: true,
        placed: true,
        job: { status: 'complete', outputPath: 'Videos/p/motion/scene.webm' },
        scene: { renderStatus: 'complete', outputAssetId: 'asset_1' },
      },
    },
    ...visualQaEvidence(),
    ...completedRenderEvidence(),
  ], contract);
  assert.equal(result.motionSceneCreate, true);
  assert.equal(result.motionSceneRender, true);
  assert.equal(result.motionSceneStatus, true);
  assert.equal(result.motionSceneComplete, true);
  assert.equal(result.complete, true);
});
...
test('media lane alone never enables the video contract (image-article regression)', () => {
  // The fighter-jets incident: an image+article run on the media lane got the
  // [SYSTEM CONTINUATION — VIDEO RUN INCOMPLETE] checklist injected mid-run.
  const imageRun = inferVideoExecutionContract({
    task: "create an article on types of fighter jets, include images, images in Claude Monet's style",
    context: { executionLane: 'media' },
  });
  assert.equal(imageRun.enabled, false);

  // Media lane + real video vocabulary still enables.
  const videoRun = inferVideoExecutionContract({
    task: 'edit this podcast video into three TikTok clips',
    context: { executionLane: 'media' },
  });
  assert.equal(videoRun.enabled, true);

  // The deterministic saved-workflow flag still wins without video wording
  // (execution verb still required, as before).
  const flagged = inferVideoExecutionContract({
    task: 'produce the deliverables for the saved workflow',
    context: { videoExecution: true },
  });
  assert.equal(flagged.enabled, true);
});

test('installed-skill NAMES in the prompt cannot arm the video contract', () => {
  // Exact production misfire: a Notion-app coding step whose prompt lists the
  // agent's installed video skills got the full video continuation checklist.
  const contract = inferVideoExecutionContract({
    task: [
      '[TASK: "i want you to create me a notion style app instead" | STEP 1/4: "Define MVP scope"]',
      '**You are Ren** — Product Builder.',
      'Skills loaded for this step (follow them): Explainer Video, Long Video to Shorts, Multi Clips to Reels, Talking Head Edit, AI Cinematic Short Film.',
      'Research the core Notion experience and define what our MVP should include. Output SCOPE.md.',
    ].join('\n'),
    systemPrompt: 'Installed skills: Trooper Video Director, Trooper Remotion Expert. Use video_edit for video work.',
    context: { executionLane: 'code' },
  });
  assert.equal(contract.enabled, false);
});

test('the system prompt and context payload never feed video classification', () => {
  const contract = inferVideoExecutionContract({
    task: 'Create a landing page for our product.',
    systemPrompt: 'video_edit video_render_start TikTok Reels — video SOP block always present.',
    context: {
      executionLane: 'code',
      installedSkills: [{ name: 'Long Video to Shorts' }, { name: 'Multi Clips to Reels' }],
    },
  });
  assert.equal(contract.enabled, false);
});

test('videoExecution:false is a hard veto even for video-worded tasks', () => {
  const contract = inferVideoExecutionContract({
    task: 'Create a TikTok from this podcast clip.',
    context: { executionLane: 'media', videoExecution: false },
  });
  assert.equal(contract.enabled, false);
  assert.equal(contract.maxContinuationAttempts, 0);
});

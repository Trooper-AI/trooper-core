 ...
 * webhook arrival → event normalization → SSE streaming → callback to Trooper
 *
 * Tests the contract between Trooper and openclawbridge
...
test('Scenario A1: bridge event sequence carries stable versioned envelope', () => {
  const sessionKey = 'agent:main:hook:trooper:build-page:task:task-001';
...
test('Scenario C1: sub-agent events carry parent/child session metadata', () => {
  const parentSessionKey = 'agent:main:hook:trooper:task-1:chat';
  const parentRunId = 'run-parent';
  const childSessionKey = 'agent:spc-dev:hook:trooper:task-1:step-2';
...
test('Scenario E1: webhook request body has all required contract fields', () => {
  // This is what Trooper sends to POST /webhook/trooper (or /webhook/mission-control)
  const payload = {
...
test('Scenario E2: callback payload to Trooper has correct shape', () => {
  // This is what the bridge sends to POST /api/agent-response on Trooper
...
test('Scenario F2: browser session payload for live VNC view', () => {
  const payload = buildBrowserSessionPayload({
    liveViewUrl: 'https://org-abc.trooper.so/vnc/vnc.html',
    sessionId: 'vnc-session-1',
    domain: 'staging.example.com',
    provider: 'openclaw',
  });

  assert.equal(payload.liveViewUrl, 'https://org-abc.trooper.so/vnc/vnc.html');

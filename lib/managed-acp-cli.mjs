...
export const MANAGED_ACP_CLI_CATALOG = Object.freeze({
  claude: Object.freeze({
    ...
    binary: 'claude',
    authMode: 'interactive_host_login',
    authHint: 'Claude Code must be signed in as the gateway node user. Trooper does not yet claim a device-code browser handoff for Claude.',
  ...
  gemini: Object.freeze({
    ...
    binary: 'gemini',
    authMode: 'credential_config_required',
    authHint: 'Gemini CLI needs a configured Google credential or API key on the gateway. There is no generic device-code handoff here.',
  }),
  opencode: Object.freeze({
    ...
    authHint: 'OpenCode requires a deliberate provider selection before login. Trooper will not choose or submit a provider credential for you.',
  }),
});
...
/**
 * Performs fixed install/probe operations for the three package-based ACP
 * harnesses. Authentication remains provider-specific: only Claude has a
 * status probe, and none of these routes accepts credentials or browser URLs.
 */
  ...
  async getAvailability(harness) {
    ...
    if (installed && recipe.statusArgs) {
      try {
        ...
        const output = redactManagedAcpCliOutput(`${result?.stdout || ''}\n${result?.stderr || ''}`, 800);
        authenticated = commandSucceeded(result) ? parseClaudeAuthStatus(output) : false;
        if (!commandSucceeded(result)) authError = output || 'CLI authentication is required';
      } catch (error) {
        authenticated = false;

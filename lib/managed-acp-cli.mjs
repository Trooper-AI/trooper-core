...
export const MANAGED_ACP_CLI_CATALOG = Object.freeze({
  claude: Object.freeze({
    ...
    authHint: 'Connect an eligible Claude account with Claude Code’s official browser login. API credentials are available only under Advanced.',
  ...
  gemini: Object.freeze({
    ...
    authHint: 'Personal Gemini is not exposed as an ACP. Enterprise Google Cloud credentials remain available under Advanced.',
    exposed: false,
  }),
  opencode: Object.freeze({
    label: 'OpenCode',
    packageName: 'opencode-ai',
    packageVersion: '1.14.48',
    binary: 'opencode',
    binaryPath: '/usr/local/bin/opencode',
    authMode: 'provider_account_login',
    authHint: 'Choose ChatGPT, SuperGrok/xAI, or GitHub Copilot and complete the provider’s official account login.',
  }),
  kimi: Object.freeze({
    label: 'Kimi Code',
    ...
    authHint: 'Connect a Kimi account with the official device-code flow.',
  }),
  copilot: Object.freeze({
    ...
    authMode: 'device_code',
    authHint: 'Connect an eligible GitHub Copilot account with GitHub’s official device login.',
  }),
});
...
export function buildManagedAcpCliDockerArgs({ containerName, harness, action = 'probe' } = {}) {
  ...
  if (action === 'install') {
    ...
    return [
      ...
      ...prefix,
      'npm', 'install', '--global', '--no-audit', '--no-fund', `${recipe.packageName}@${recipe.packageVersion}`,
    ];
  }
  if (action === 'auth_status' && recipe.statusArgs) {
    return [...common, ...prefix, recipe.binaryPath, ...recipe.statusArgs];
  }
  if (action === 'probe') return [...common, ...prefix, recipe.binaryPath, '--version'];

...
export function redactCodexDeviceAuthOutput(value, limit = MAX_CODEX_DEVICE_AUTH_OUTPUT_CHARS) {
  ...
  text = text.replace(
    /(["']?(?:access_token|refresh_token|id_token|session_token|token|api[_-]?key|authorization|secret|password)["']?\s*[:=]\s*["']?)([^\s,"'}\]]+)/gi,
    '$1[redacted]',
  );
...
export function parseCodexDeviceAuthInstructions(value) {
  ...
  const codeMatch = output.match(
    /(?:device|verification|user|authorization)?\s*code\s*(?:is|:)?\s*[`"']?([A-Z0-9]{4,}(?:[- ][A-Z0-9]{2,}){0,5})[`"']?/i,
  );
  const rawCode = String(codeMatch?.[1] || '').trim().replace(/\s+/g, ' ');
  const userCode = rawCode && rawCode.length <= 48 ? rawCode : null;
...
export class CodexDeviceAuthJobManager {
  ...
  async getAvailability() {
    ...
    try {
      ...
      const authenticated = commandSucceeded(result) && /logged in|authenticated|chatgpt/i.test(statusOutput);
      return {
        installed: true,
        authenticated,
        version,
  ...
  async handleChildClose(job, kind, code, signal) {
    ...
    if (code !== 0) {
      this.finish(job, 'failed', { error: `Codex device authorization exited with ${signal || code || 'an error'}` });
      return;
    }
    ...
    this.finish(job, 'failed', {
      error: availability.error || 'Codex did not report an authenticated gateway session after device authorization',
    });
  }

  async start({ force = false, installIfMissing = true } = {}) {
    const active = this.currentActiveJob();
    if (active) {

...
test('writePluginFiles writes plugin bundle to host plugin directory', () => {
  ...
  const unchanged = writePluginFiles({
    pluginId: 'trooper-composio-bridge',
    pluginsRoot,
    files: [
      { path: 'index.js', content: 'export default {};\n' },
...
test('installOpenClawPlugin runs docker exec with allowlisted plugin path', () => {
  ...
  assert.equal(output.containerMountedPath, '/opt/openclaw-data/plugins/trooper-composio-bridge');
  assert.equal(output.containerPluginPath, '/tmp/openclaw-plugin-trooper-composio-bridge');
  assert.match(calls[0], /docker exec -u 0 openclaw-openclaw-gateway-1 sh -lc/);
  assert.match(calls[0], /chown -R/);
  assert.match(calls[1], /docker exec -u node openclaw-openclaw-gateway-1 openclaw plugins install/);
  assert.match(calls[1], /\/opt\/openclaw-data\/plugins\/trooper-composio-bridge/);
});

test('installOpenClawPlugin falls back to copied plugin path with diagnostics', () => {
  const calls = [];
  const output = installOpenClawPlugin({
    pluginPath: '/opt/openclaw-data/plugins/trooper-composio-bridge',
    execSync: (cmd) => {
      calls.push(cmd);
      if (cmd.includes('openclaw plugins install') && cmd.includes('/opt/openclaw-data/plugins')) {
        const error = new Error('mounted path missing');
        error.stderr = 'not found';
        throw error;
      }
      return 'installed tmp copy';
    },
  });

  assert.equal(output.installed, true);
  assert.equal(output.installAttempts.length, 2);
  assert.equal(output.installAttempts[0].ok, false);
  assert.equal(output.installAttempts[1].ok, true);
  assert.ok(calls.some((cmd) => cmd.includes('docker cp')));
  assert.ok(calls.some((cmd) => cmd.includes('/tmp/openclaw-plugin-trooper-composio-bridge')));
});
...
test('syncGatewayPlugin force-overwrites the gateway extensions copy', () => {
  const pluginsRoot = mkdtempSync(path.join(os.tmpdir(), 'bridge-plugins-sync-ow-'));
  const calls = [];
  const result = syncGatewayPlugin({
    pluginId: 'trooper-video-bridge',
    pluginsRoot,
    files: [
      { path: 'index.js', content: 'export default {};\n' },
      { path: 'openclaw.plugin.json', content: '{"contracts":{"tools":[]}}\n' },
    ],
    mkdirSync,
    writeFileSync,
    execSync: (cmd) => {
  ...
  assert.equal(result.extensionsOverwritten, true);
  assert.equal(result.extensionsDir, '/home/node/.openclaw/extensions/trooper-video-bridge');
  assert.ok(calls.some((cmd) => cmd.startsWith('docker cp ')
    && cmd.includes('trooper-video-bridge/.')
    && cmd.includes(':/home/node/.openclaw/extensions/trooper-video-bridge/')));
});

test('syncGatewayPlugin survives a failing CLI install when the overwrite lands', () => {
  const pluginsRoot = mkdtempSync(path.join(os.tmpdir(), 'bridge-plugins-sync-fail-'));
  const result = syncGatewayPlugin({
    pluginId: 'trooper-video-bridge',
    pluginsRoot,
    files: [{ path: 'openclaw.plugin.json', content: '{}\n' }],
    mkdirSync,
    writeFileSync,
    execSync: (cmd) => {
      if (cmd.includes('openclaw plugins install')) {
  ...
  assert.match(result.installError, /already installed/);
  assert.equal(result.extensionsOverwritten, true);
});

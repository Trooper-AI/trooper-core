...
test('installOpenClawPlugin runs docker exec with allowlisted plugin path', () => {
  ...
  assert.match(calls[0], /docker exec openclaw-openclaw-gateway-1 rm -rf/);
  assert.match(calls[1], /docker cp/);
  assert.match(calls[2], /docker exec -u 0 openclaw-openclaw-gateway-1 sh -lc/);
  assert.match(calls[2], /chown -R/);
  assert.match(calls[3], /docker exec -u node openclaw-openclaw-gateway-1 openclaw plugins install/);
  assert.match(calls[3], /\/tmp\/openclaw-plugin-trooper-composio-bridge/);
});

...
test('upgrade journal records the failed scoped endpoints before rollback', () => {
  ...
  assert.match(source, /upgrade failed health verification \(\$\{failed_endpoints\}\)/);
});

test('upgrade verifier polls health without blocking on systemd stop timeout', () => {
  assert.match(source, /systemctl restart --no-block/);
  assert.match(source, /TROOPER_UPGRADE_HEALTH_ATTEMPTS:-150/);
});

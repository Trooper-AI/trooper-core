...
const OPENCLAW_CONFIG_PATH = process.env.OPENCLAW_CONFIG_PATH
  || `${process.env.OPENCLAW_CONFIG_ROOT || `${process.env.OPENCLAW_DATA_ROOT || '/opt/openclaw-data'}/config`}/openclaw.json`;
const DEFAULT_MODEL_CACHE_TTL_MS = 5000;
const OPENAI_CODEX_RESPONSES_API = 'openai-chatgpt-responses';
// OpenClaw 2026.6+ Codex transport posts to /backend-api/codex/responses.
// Using bare /backend-api triggers Cloudflare challenge HTML (403) and auth failures.
...
export function normalizeProviderErrorMessage(message = '', { provider = null, model = null } = {}) {
  ...
  // ChatGPT Plus rate limit (Codex backend) — often mis-surfaced as opaque 400/auth errors.
  if (/usage_limit_reached|usage limit has been reached|hit your chatgpt usage limit/i.test(lower)) {
    const resetMatch = raw.match(/resets_in_seconds["\s:]+(\d+)/i)
      || raw.match(/try again in ~?(\d+)\s*min/i);
    const mins = resetMatch
      ? (raw.includes('min') ? Number(resetMatch[1]) : Math.ceil(Number(resetMatch[1]) / 60))
      : null;
    const when = Number.isFinite(mins) ? ` Try again in ~${mins} minutes.` : ' Wait for the ChatGPT Plus rate limit to reset, or add an OpenAI API key / another provider.';
    return `ChatGPT Plus Codex usage limit reached.${when} Setup is fine (OAuth is loaded); this is an account quota, not a dead gateway.`;
  }

  if (isCodex && /400 status code \(no body\)|missing bearer or basic authentication/i.test(lower)) {
    return `${raw} If Codex OAuth is connected, this is often a ChatGPT usage-limit or wrong model provider path (must be openai-codex/*, not openai/*). Re-check Server Console Auth Profiles and ChatGPT plan limits.`;
  }

...
/**
 * Normalize a skill slug for volume paths (no host package installs).
 */
...
/**
 * Install one skill as files on a workspace volume.
 * Density P5: skills are data on the org slot volume — never apt/host mutations.
 *
 * @returns {{ ok: boolean, slug?: string, skillDir?: string, written?: string[], error?: string }}
 */
export function installSkillFilesToVolume(skillRoot, {
  slug = '',
  content = '',
  files = null,
} = {}) {
  ...
  if (!root) return { ok: false, error: 'skill_root_required' };
  if (!safeSlug) return { ok: false, error: 'slug_required' };

  const skillDir = path.join(root, safeSlug);
  // Prevent path escape
  if (!skillDir.startsWith(path.resolve(root) + path.sep) && path.resolve(skillDir) !== path.resolve(root, safeSlug)) {
    return { ok: false, error: 'invalid_skill_path' };
  }

  mkdirSync(skillDir, { recursive: true });
  const written = [];
  const fileMap = files && typeof files === 'object' && !Array.isArray(files)
    ? { ...files }
    : {};
  if (content && !fileMap['SKILL.md'] && !fileMap['skill.md']) {
    fileMap['SKILL.md'] = content;
  }
  if (!fileMap['SKILL.md'] && !fileMap['skill.md']) {
    return { ok: false, error: 'skill_md_required' };
  }

  for (const [name, body] of Object.entries(fileMap)) {
    const base = path.basename(String(name || '').trim());
    if (!base || base === '.' || base === '..') continue;
    if (base.includes('..') || base.includes('/') || base.includes('\\')) continue;
    const filePath = path.join(skillDir, base);
    writeFileSync(filePath, normalizeContent(body), 'utf8');
    written.push(filePath);
  }

  return { ok: true, slug: safeSlug, skillDir, written };
}

/**
 * List skills present on a volume (directories with SKILL.md).
 */
export function listVolumeSkills(skillRoot) {
  const root = String(skillRoot || '').trim();
  ...
  for (const entry of readdirSync(root)) {
    ...
    const skillMdAlt = path.join(dir, 'skill.md');
    if (existsSync(skillMd) || existsSync(skillMdAlt)) {
      out.push({
        slug: entry,
        path: dir,
        skillMd: existsSync(skillMd) ? skillMd : skillMdAlt,
      });
    }
  }
...
/**
 * Remove a skill directory from the volume (idempotent).
 */
export function removeSkillFromVolume(skillRoot, slug = '') {
  const root = String(skillRoot || '').trim();
  const safeSlug = normalizeSkillSlug(slug);
  if (!root || !safeSlug) return { ok: false, error: 'invalid_args', removed: false };
  const skillDir = path.join(root, safeSlug);
  const resolvedRoot = path.resolve(root);
  const resolvedDir = path.resolve(skillDir);
  if (!resolvedDir.startsWith(`${resolvedRoot}${path.sep}`) && resolvedDir !== path.join(resolvedRoot, safeSlug)) {
    return { ok: false, error: 'invalid_skill_path', removed: false };
  }
  if (!existsSync(skillDir)) return { ok: true, removed: false, slug: safeSlug };
  rmSync(skillDir, { recursive: true, force: true });
  return { ok: true, removed: true, slug: safeSlug, skillDir };
}

/**
 * Canonical skill roots on a shared workspace volume (pool slot / VPS workspace).
 */
export function resolveWorkspaceSkillRoots({ workspaceRoot = '', configRoot = '' } = {}) {
  const roots = [];
  if (workspaceRoot) roots.push(path.join(String(workspaceRoot), 'skills'));
  if (configRoot) roots.push(path.join(String(configRoot), '.agents', 'skills'));
  return [...new Set(roots.map((r) => path.resolve(r)))];
}

/**
 * Ensure default skill pack exists on all volume skill roots for a slot.
 */
export function ensureVolumeSkillPacks({ workspaceRoot = '', configRoot = '' } = {}) {
  const roots = resolveWorkspaceSkillRoots({ workspaceRoot, configRoot });
  const results = [];
  for (const root of roots) {
    const written = ensureDefaultSkillPack(root);
    results.push({
      root,
      writtenCount: written.length,
      skillCount: listVolumeSkills(root).length,
    });
  }
  return results;
}

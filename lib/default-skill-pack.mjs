...
`),
  },
  {
    ...
    content: normalizeContent(`
---
summary: Send and read email as this agent through the bridge /email endpoints; every agent has an assigned address.
whenToUse: Use when a task involves reading your inbox, replying to an email thread, or sending email as yourself. Check IDENTITY.md for your address.
allowedTools:
  - exec
keywords:
  - email
...
You have your own email address (see IDENTITY.md). The local bridge exposes your mailbox — all requests need the bridge token:
...
## Read your inbox
\`\`\`bash
curl -s -H "$AUTH" "$BASE/email/inbox?agentName=<your name>&unreadOnly=true&limit=10"
\`\`\`
Returns threads with \`id\`, \`subject\`, \`lastFrom\`, \`lastSnippet\`, \`unread\`.

## Read a full thread
\`\`\`bash
curl -s -H "$AUTH" "$BASE/email/thread/<threadId>?agentName=<your name>"
\`\`\`

## Send or reply
\`\`\`bash
curl -s -X POST -H "$AUTH" -H "Content-Type: application/json" "$BASE/email/send" -d '{
  "agentName": "<your name>",
  "to": ["person@example.com"],
  "subject": "Subject (omit when replying to a thread)",
  "text": "Plain-text body",
  "replyToThreadId": "<threadId when replying>"
}'
\`\`\`
Replies thread automatically (In-Reply-To/References are set server-side). Your assigned address is always the sender — you cannot send as anyone else.

## Deliverable Rules
- Never fabricate email contents, senders, or recipients — quote only what you actually read.
- Preview outbound emails to the human when the send is consequential (external recipients, commitments, money).
- A 403 means this agent's email permission is restricted — report it, don't work around it.
- When you reply to a thread, say so and include the thread id in your summary.
`),
  },
];

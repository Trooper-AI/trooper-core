...
export function installOpenClawPlugin({
  ...
  try {
    output = execSync(
      `docker exec -u node ${containerName} openclaw plugins install ${shellQuote(containerPluginPath)} 2>&1`,
      { timeout: 120000, encoding: 'utf8' },
    );
  } catch (error) {
    const detail = commandErrorDetails(error);
    const next = new Error(`OpenClaw local plugin install failed for ${id}: ${detail}`);

...
import { extractStructuredToolResult, forwardToolEventPayload } from './event-contracts.mjs';
import { markRunEnded, markRunStarted, touchRunActivity } from './run-watchdog.mjs';
import { maybeSpillToolResultText } from './tool-result-spill.mjs';
import {
...
/** Mirror Trooper complex-request gate (keep logic local — bridge may deploy alone). */
function assessComplexChatRequest(text = '') {
  const raw = String(text || '').trim();
  if (!raw || raw.length < 24) return { shouldPlanFirst: false, score: 0 };
  if (/\b(?:just do it|don'?t plan|no plan|skip planning|execute now)\b/i.test(raw)) {
    return { shouldPlanFirst: false, score: 0 };
  }
  let score = 0;
  if (raw.length >= 280) score += 2;
  if (raw.length >= 700) score += 2;
  if (/(?:^|\n)\s*(?:\d+[.)]\s+|[-*]\s+).+(?:\n\s*(?:\d+[.)]\s+|[-*]\s+).+){1,}/m.test(raw)) score += 3;
  if (/\b(?:step[- ]by[- ]step|multi[- ]step|phased|roadmap)\b/i.test(raw)) score += 3;
  if (/\b(?:build|create|implement|develop|ship|launch)\b/i.test(raw)
    && /\b(?:and then|then also|after that|followed by)\b/i.test(raw)) score += 2;
  if (/\b(?:build|create|implement)\b/i.test(raw)
    && /\b(?:videos?|tiktoks?|reels?|pages?|apps?|workflows?|features?)\b/i.test(raw)) score += 2;
  const actions = raw.match(/\b(?:research|design|write|edit|render|deploy|test|verify|publish|import|cut|caption|package)\b/gi) || [];
  if (new Set(actions.map((v) => v.toLowerCase())).size >= 3) score += 3;
  return { shouldPlanFirst: score >= 4, score };
}

function buildAutoPlanFirstDirective(score = 0) {
  return `[STRUCTURED PLAN FIRST — auto for complex work]
This request looks multi-step (score=${score}).
Before state-changing tools (write, edit, video_render_*, deploy):
1. Outline a short concrete plan (3–8 steps) in chat
2. Call update_plan (or write_todos if available) with those steps
3. Prefer reading a matching workspace/skills/*/SKILL.md before freestyling
4. Execute step by step and update the plan as you go
Do not wait for human approval unless blocked. Trivial one-tool work may skip this.`;
}

function skillIdFromSkillMdPath(filePath = '') {
  const raw = String(filePath || '').trim().replace(/\\/g, '/');
  if (!raw || !/SKILL\.md$/i.test(raw)) return null;
  const m = raw.match(/(?:^|\/)(?:skills|workspace\/skills)\/([^/]+)\/SKILL\.md$/i)
    || raw.match(/\/([^/]+)\/SKILL\.md$/i);
...
export async function handleChatMessage(msg, user, ws, ctx) {
  ...
  // ── 5. Build context ─────────────────────────────────────────────────────
  let systemPrompt = buildSystemPrompt(agent, channel, {
  ...
  // Auto plan-first for complex freeform chat (unless user forced execute/plan UI flags).
  const collabMode = String(msg.collaborationMode || '').toLowerCase();
  const forceExecute = msg.executeMode === true || collabMode === 'execute';
  const forcePlan = msg.planMode === true || collabMode === 'plan';
  if (!forceExecute && !forcePlan) {
    const complexity = assessComplexChatRequest(msg.content || '');
    if (complexity.shouldPlanFirst) {
      const planDirective = buildAutoPlanFirstDirective(complexity.score);
  ...
  const gatewayAgentId = isSPC
    ...
    : 'main';
  // Prefer Trooper chatSessionId so the UI can rotate after context overflow /new.
  // Falling back to channel:{name} reuses one heavy OpenClaw transcript forever.
  const rawChatSessionId = msg.chatSessionId
    || msg.context?.chatSessionId
    || msg.conversationSessionId
    || null;
  ...
  const sessionKey = chatSessionId
    ? `agent:${gatewayAgentId}:hook:trooper:${agentSlugForSession}:chat-session:${chatSessionId}`
    : `agent:${gatewayAgentId}:hook:trooper:${agentSlugForSession}:channel:${channel}`;

  // Liveness mark for the orphan-run watchdog (lib/run-watchdog.mjs) — an
  // in-memory record that this process owns the run right now.
  markRunStarted(runId, { sessionKey, channel, agentId: agent.id, agentName: agent.name });
  ...
  /** Pull token usage from gateway result / stream payloads (handles snake + camel + breakdown). */
  const ingestUsagePayload = (usage, meta = {}) => {
    if (!usage || typeof usage !== 'object') return false;
    const beforeIn = inputTokens;
    const beforeOut = outputTokens;
    const rows = Array.isArray(usage.modelBreakdown)
    ...
    if (rows?.length) {
      ...
      for (const row of rows) {
        const rowUsage = row?.usage && typeof row.usage === 'object' ? row.usage : row;
        const i = Number(rowUsage?.inputTokens ?? rowUsage?.input_tokens ?? rowUsage?.prompt_tokens ?? rowUsage?.input ?? 0);
        const o = Number(rowUsage?.outputTokens ?? rowUsage?.output_tokens ?? rowUsage?.completion_tokens ?? rowUsage?.output ?? 0);
        if (Number.isFinite(i) && i > 0) inSum += Math.round(i);
        if (Number.isFinite(o) && o > 0) outSum += Math.round(o);
      }
      if (inSum > 0) inputTokens = Math.max(inputTokens, inSum);
      if (outSum > 0) outputTokens = Math.max(outputTokens, outSum);
    }
    const i = Number(usage.inputTokens ?? usage.input_tokens ?? usage.prompt_tokens ?? usage.input ?? 0);
    const o = Number(usage.outputTokens ?? usage.output_tokens ?? usage.completion_tokens ?? usage.output ?? 0);
    if (Number.isFinite(i) && i > 0) inputTokens = Math.max(inputTokens, Math.round(i));
    if (Number.isFinite(o) && o > 0) outputTokens = Math.max(outputTokens, Math.round(o));
    const improved = inputTokens > beforeIn || outputTokens > beforeOut;
    // Persist a timeline done/model_done so ActivityDrawer can price the run (once, unless improved).
    if ((inputTokens > 0 || outputTokens > 0) && (improved || !usageEventsEmitted)) {
      usageEventsEmitted = true;
      const usageEnvelope = {
        input_tokens: inputTokens,
        output_tokens: outputTokens,
        total_tokens: inputTokens + outputTokens,
        ...(usage.modelBreakdown ? { modelBreakdown: usage.modelBreakdown } : {}),
        ...(usage.costUsd != null ? { costUsd: usage.costUsd } : {}),
        ...(usage.estimated != null ? { estimated: usage.estimated } : {}),
        ...(usage.source ? { source: usage.source } : {}),
        model: meta.model || usage.model || null,
        provider: meta.provider || usage.provider || null,
      };
      emitBridgeToolEvent('model_done', {
        model: usageEnvelope.model,
        provider: usageEnvelope.provider,
        usage: usageEnvelope,
      });
      emitBridgeToolEvent('done', { usage: usageEnvelope });
    }
  ...
  const streamCallback = (stream, data, gwRunId) => {
    touchRunActivity(runId); // any stream event proves the run is alive
    ...
    if (stream === 'tool_start' && data) {
      ...
      const toolStartPayload = forwardToolEventPayload('tool_start', data, {
        tool: toolName,
        params: toolParams,
      });
      // Skill telemetry: SKILL.md reads and load_skill show as skill_start in Activity.
      const skillFromRead = /^(?:read|read_file)$/i.test(toolName)
        ? skillIdFromSkillMdPath(toolParams.path || toolParams.file_path || toolParams.filePath || '')
        : null;
      const skillFromLoad = /^load_skill$/i.test(toolName)
        ? String(toolParams.skillId || toolParams.skill_id || toolParams.slug || toolParams.name || '').trim()
        : '';
      const skillId = skillFromRead || skillFromLoad || null;
      if (skillId) {
        const skillEvent = {
          tool: 'skill_start',
          skillId,
          skillName: skillId,
          name: skillId,
          source: skillFromLoad ? 'load_skill' : 'skill_md_read',
          path: skillFromRead ? (toolParams.path || toolParams.file_path || '') : undefined,
          summary: `Using skill · ${skillId}`,
          sessionKey,
          time: Date.now(),
        };
        bridgeWS.broadcast('agent:tool_event', {
        ...
        allEvents.push({
          seq: seq++,
          event: 'skill_start',
          data: JSON.stringify(skillEvent),
          timestamp: Date.now(),
        });
      }
      bridgeWS.broadcast('agent:tool_event', {
      ...
      allEvents.push({
        seq: seq++,
        event: 'tool_start',
        data: JSON.stringify(toolStartPayload),
        timestamp: Date.now(),
      });
    }
    ...
    if (stream === 'tool_result' && data) {
      ...
      // Oversized raw text is externalized at this persistence point
      // (lib/tool-result-spill.mjs); summary above was already computed from
      // the full text, so the UI keeps its excerpt either way. When spilled,
      // `result` is dropped too — it is the same bytes in parsed form, and
      // persisting it inline would defeat the spill (the locator carries the
      // full body's location and size).
      const spillOutcome = maybeSpillToolResultText(rawSerialized);
      const toolResultPayload = forwardToolEventPayload('tool_result', data, {
        tool: toolName,
        result: spillOutcome.spilled ? null : structuredResult,
        raw: spillOutcome.text,
        success,
        summary,
      });
      if (spillOutcome.spilled) toolResultPayload.spill = spillOutcome.locator;
      bridgeWS.broadcast('agent:tool_event', {
        agentId: agent.id,
        agentName: agent.name,
        event: 'tool_result',
      ...
      allEvents.push({
        seq: seq++,
        event: 'tool_result',
        data: JSON.stringify(toolResultPayload),
        timestamp: Date.now(),
      });
    }

    // Terminal usage events from runAgentStreaming (history fetch + char estimate)
    if (stream === 'model_done' || stream === 'done') {
      const usage = data?.usage && typeof data.usage === 'object' ? data.usage : data;
      // ingestUsagePayload already broadcasts + records timeline events when tokens improve
      ingestUsagePayload(usage, {
        model: data?.model || usage?.model || null,
        provider: data?.provider || usage?.provider || null,
      });
    }
  ...
  try {
    ...
    // Capture token usage from gateway result (was previously left at 0 → "pricing unavailable").
    ingestUsagePayload(result?.usage, {
      model: result?.model || agent.model || null,
      provider: result?.provider || agent.provider || null,
    });
    // Char-estimate last resort so completed chat runs still show an approximate cost.
    if (inputTokens <= 0 && outputTokens <= 0 && finalText) {
      const estOut = Math.max(1, Math.ceil(finalText.length / 3.5));
      const estIn = Math.max(1, Math.ceil(String(userMessage || '').length / 3.5) + toolCount * 200);
      ingestUsagePayload({
        input_tokens: estIn,
  ...
  try {
    ...
    if (intent.type === 'list_devices') {
      emitToolEvent('tool_result', {
        ...
        success: true,
        summary: 'Listed paired devices from Trooper runtime',
      });
      return {
        handled: true,
    ...
    if (!targetDevice) {
      ...
      return {
        handled: true,
        finalText: 'I recognized that as a paired-device request, but I could not find a matching approved device in the Trooper runtime.',
      };
    }

    const commands = intent.type === 'safe_command'
      ? (Array.isArray(intent.commands) && intent.commands.length ? intent.commands : [intent.command])
      : intent.commands || [];
    ...
    return {
      handled: true,
      finalText: `I recognized that as a paired-device request, but the Trooper device runtime is unavailable right now: ${error.message}`,
    };
  }
}

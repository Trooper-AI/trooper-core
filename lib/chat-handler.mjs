      ...
      (stream, data, gwRunId) => {
        ...
        if (stream === 'tool_result' && data) {
          ...
          bridgeWS.broadcast('agent:tool_event', {
            ...
            agentName: agent.name,
            event: 'tool_result',
            data: { tool: data.name || 'unknown', success: !data.is_error, summary },
            runId,
          });
          allEvents.push({
            seq: seq++,
            event: 'tool_result',
            data: JSON.stringify({ tool: data.name || 'unknown', success: !data.is_error, summary }),
            timestamp: Date.now(),
          });
        }
...
function buildSystemPrompt(agent, channel, companyDocs) {
  const companyName = companyDocs?.match(/^#\s+(.+)$/m)?.[1]?.trim() || 'the company';

  let prompt = `[CONTEXT] You are ${agent.name}, ${agent.title || agent.role || 'Team Member'} at ${companyName}. You're responding in the team chat channel "${channel}".`;
  ...
  try {
    ...
    if (activeTasks.length > 0) {
      ...
      for (const t of activeTasks) {
        prompt += `- [${t.status}] ${t.title}${t.assignee_name ? ` (assigned: ${t.assignee_name})` : ''}\n`;
      }
    }
  } catch (_) { /* no tasks table or query error — skip */ }

  prompt += `\n\n## HOW TO RESPOND
- Actually USE your tools. Don't describe what you'd do — do it.
- If asked to search, fetch, exec, browse — try them in sequence if needed.
- Show your work as you go. Stream updates.
- Only give a final answer after you've actually tried.
- Be concise. No fluff.`;

...
import { joinExpressSplat } from './express-splat.mjs';

const WORKSPACE_APPS_DIR = process.env.OPENCLAW_WORKSPACE_APPS_DIR
  || `${process.env.OPENCLAW_WORKSPACE_HOST_ROOT || `${process.env.OPENCLAW_DATA_ROOT || '/opt/openclaw-data'}/workspace`}/apps`;
...
export function registerWorkspaceAppRoutes(app) {
  ...
  app.get('/api/apps/:slug/{*assetPath}', (req, res) => {
    try {
      const relativePath = joinExpressSplat(req.params.assetPath ?? req.params[0]);

...
import { registerDataApiRoutes } from './data-workspace.mjs';
import { registerWorkspaceAppRoutes } from './workspace-apps.mjs';
import { registerMessageRoutes } from './message-handler.mjs';
import {
  listMemoryEntries,
  listMemorySources,
  searchMemoryEntries,
...
function openclawDataPath(...segments) {
  return [OPENCLAW_DATA_ROOT, ...segments.filter(Boolean)].join('/').replace(/\/+/g, '/');
}

function openclawConfigPath(...segments) {
  return [OPENCLAW_CONFIG_ROOT, ...segments.filter(Boolean)].join('/').replace(/\/+/g, '/');
}
...
export function registerApiRoutes(app, { agentRegistry, gateway, bridgeWS, getCompanyDocs, setCompanyDocs }) {
  ...
  registerMessageRoutes(app, { bridgeWS });
  ...
  app.get('/api/models', (req, res) => {
    ...
    // Include local-llamacpp models from openclaw.json if configured
    try {
      const config = JSON.parse(readFileSync(OPENCLAW_CONFIG_PATH, 'utf8'));
      const localProvider = config?.models?.providers?.['local-llamacpp'];
      if (localProvider?.models && Array.isArray(localProvider.models)) {
        for (const m of localProvider.models) {
          models.push({
      ...
      if (ollamaProvider?.models && Array.isArray(ollamaProvider.models)) {
        for (const m of ollamaProvider.models) {
          const id = String(m.id || '').replace(/^ollama\//, '');
          if (!id) continue;
          models.push({
            id: `ollama/${id}`,
            name: `${m.name || id} (Ollama)`,
            provider: 'ollama',
            contextWindow: m.contextWindow || DEFAULT_LOCAL_OLLAMA_CONTEXT_WINDOW,
            isLocal: true,
          });
        }
      }
    } catch { /* ignore — config may not exist or have no local provider */ }
  ...
  // DELETE /api/messages — handled by registerMessageRoutes()
  ...
  app.get('/api/capabilities/voice', (req, res) => {
    ...
    res.json({
      tts: hasOpenAIKey,
      stt: hasOpenAIKey || hasOpenRouterKey,
      sttProvider: hasOpenRouterKey ? 'openrouter' : (hasOpenAIKey ? 'openai' : null),
      sttProviders: {
  ...
  // ── Humans ────────────────────────────────────────────────────────────────
  // Collapse duplicate person records (same firebaseUid, email, or name under
  // different ids) that older builds and cross-runtime syncs left behind —
  // they rendered the same human several times in the org chart.
  const humanIdentityKeys = (human) => {
    const keys = [];
    const uid = String(human?.firebaseUid || '').trim();
    const email = String(human?.email || '').trim().toLowerCase();
    const name = String(human?.name || '').trim().toLowerCase();
    if (uid) keys.push(`uid:${uid}`);
    if (email) keys.push(`email:${email}`);
    if (name) keys.push(`name:${name}`);
    return keys;
  };
  const dedupeHumanRecords = (humans) => {
    const keeperByKey = new Map();
    const list = [];
    let removed = 0;
    for (const human of Array.isArray(humans) ? humans : []) {
      const keys = humanIdentityKeys(human);
      const keeper = keys.map((key) => keeperByKey.get(key)).find(Boolean);
      if (!keeper) {
        for (const key of keys) keeperByKey.set(key, human);
        list.push(human);
        continue;
      }
      if (human?.email && !keeper.email) keeper.email = human.email;
      if (human?.firebaseUid && !keeper.firebaseUid) keeper.firebaseUid = human.firebaseUid;
      if (human?.avatar && !keeper.avatar) keeper.avatar = human.avatar;
      for (const key of humanIdentityKeys(keeper)) keeperByKey.set(key, keeper);
      removed += 1;
    }
    return { list, removed };
  };
  ...
  app.get('/api/humans', (req, res) => {
    ...
    const { list, removed } = dedupeHumanRecords(humans);
    if (removed > 0) {
      db.update(configTable)
        .set({ value: JSON.stringify(list), updated_at: Date.now() })
        .where(eq(configTable.key, 'humans'))
        .run();
      console.log(`[api] humans: collapsed ${removed} duplicate record(s)`);
    }
    res.json(list);
  });
  ...
  app.get('/api/health', (req, res) => {
    try {
      ...
      try {
        bridgeVersion = execSync('git -c safe.directory=/opt/openclaw-bridge -C /opt/openclaw-bridge rev-parse --short HEAD', { encoding: 'utf8', timeout: 3000 }).trim();
      } catch {}
  ...
  // Accepts either X-API-Key (Obsidian plugin) or Bearer token (Trooper frontend/proxy).
  // If the bridge has no API keys configured, allow through (single-tenant dev mode).
  function requireApiKey(req, res, next) {
    // Check X-API-Key header (Obsidian plugin)
    ...
    // Check Bearer token (Trooper frontend proxy) — trust if present (already verified by Trooper server)
    const authHeader = req.headers.authorization;
    if (authHeader && authHeader.startsWith('Bearer ')) {
      req.user = req.user || { uid: 'bearer-proxy', name: 'proxy' };
      return next();
    }

    // No API keys configured at all = dev mode, allow through
    const keysRow = db.select().from(configTable).where(eq(configTable.key, 'apiKeys')).get();
    const hasKeys = keysRow && JSON.parse(keysRow.value).some(k => k.active);
    if (!hasKeys) return next();

    return res.status(401).json({ error: 'Authentication required. Provide X-API-Key or Bearer token.' });
  }
  ...
  // ── Memory Conflicts (must be registered before /api/memories/:id) ──
    ...
    try {
      ...
      if (existing) {
        ...
        db.insert(memoriesTable).values({
          id: memId, title: rv.title || '(untitled)', summary: rv.summary || '', details: rv.details || '',
          tags: JSON.stringify(rv.tags || []), scope: rv.scope || 'org', confidence: String(rv.confidence ?? 0.8),
          source: JSON.stringify(rv.source || {}), created_at: Date.now(), updated_at: Date.now(),
        }).run();
      }

      db.update(memoryConflictsTable).set({
        status: 'resolved', resolution, resolved_version: JSON.stringify(rv), resolved_at: Date.now(),
  ...
  // ── Memory Sources / Synced Entries ─────────────────────────────────────
  // Native integrations use these endpoints to expose database sync as Memory.
  // These records are kept separate from hand-authored long-term memories.

  app.get('/api/memory/sources', requireApiKey, (req, res) => {
    try {
      res.json({ memorySources: listMemorySources(sqlite) });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/memory/sources', requireApiKey, (req, res) => {
    try {
      const source = upsertMemorySource(sqlite, req.body || {});
      res.json({ ok: true, source });
      try { bridgeWS.broadcast('memory:source_updated', { id: source.id, displayName: source.displayName }); } catch {}
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
  });

  app.get('/api/memory/sources/:sourceId/entries', requireApiKey, (req, res) => {
    try {
      const entries = listMemoryEntries(sqlite, req.params.sourceId, { limit: req.query.limit });
      res.json({ sourceId: req.params.sourceId, entries });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });

  app.post('/api/memory/sources/:sourceId/entries', requireApiKey, (req, res) => {
    try {
      const body = req.body || {};
      const result = upsertMemoryEntries(sqlite, req.params.sourceId, Array.isArray(body) ? body : body.entries);
      res.json({ ok: true, ...result });
      try { bridgeWS.broadcast('memory:entries_synced', { sourceId: req.params.sourceId, upserted: result.upserted }); } catch {}
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
  });

  app.post('/api/memory/search', requireApiKey, (req, res) => {
    try {
      const body = req.body || {};
      const results = searchMemoryEntries(sqlite, {
        query: body.query,
  ...
  app.post('/api/memory/sources/github/sync', requireApiKey, async (req, res) => {
    try {
      const body = req.body || {};
      const repos = body.repos || process.env.TROOPER_GITHUB_REPOS || '';
      const result = await syncGitHubMemorySource(sqlite, {
        sourceId: body.sourceId || 'github',
        repos,
        token: body.token || process.env.TROOPER_GITHUB_TOKEN || process.env.GITHUB_TOKEN || '',
        pages: body.pages || process.env.TROOPER_GITHUB_PAGES || 1,
      });
      res.json(result);
      try { bridgeWS.broadcast('memory:entries_synced', { sourceId: result.source.id, upserted: result.synced }); } catch {}
    } catch (err) {
      res.status(400).json({ ok: false, error: err.message });
    }
  });

  // ── Org-prefixed aliases (for Trooper proxy compatibility) ─────────
  // Trooper server proxies requests using /api/organizations/:orgId/memory paths.
  // These redirect internally to the /api/memories handlers.
  function orgMemoryProxy(req, res) {
    // Rewrite to internal path and re-dispatch via app.handle
    const orgId = req.params.orgId;
    const memoryId = req.params.memoryId;
    const suffix = req.params[0] || ''; // wildcard capture
    let newPath = '/api/memories';
    if (memoryId) newPath += `/${memoryId}`;
    if (suffix) newPath += suffix;
  ...
  app.get('/api/organizations/:orgId/memory', requireApiKey, orgMemoryProxy);
  app.post('/api/organizations/:orgId/memory', requireApiKey, orgMemoryProxy);
  app.get('/api/organizations/:orgId/memory/changes', requireApiKey, orgMemoryProxy);
  app.get('/api/organizations/:orgId/memory/markdown', requireApiKey, orgMemoryProxy);
  app.get('/api/organizations/:orgId/memory/conflicts', requireApiKey, orgMemoryProxy);
  app.get('/api/organizations/:orgId/memory/sources', requireApiKey, (req, res) => {
    req.url = `/api/memory/sources${req.originalUrl.includes('?') ? req.originalUrl.slice(req.originalUrl.indexOf('?')) : ''}`;
    app.handle(req, res);
  });
  app.post('/api/organizations/:orgId/memory/sources', requireApiKey, (req, res) => {
    req.url = '/api/memory/sources';
    app.handle(req, res);
  });
  app.post('/api/organizations/:orgId/memory/search', requireApiKey, (req, res) => {
    req.url = '/api/memory/search';
    app.handle(req, res);
  });
  app.get('/api/organizations/:orgId/memory/sources/:sourceId/entries', requireApiKey, (req, res) => {
    const qs = req.originalUrl.includes('?') ? req.originalUrl.slice(req.originalUrl.indexOf('?')) : '';
    req.url = `/api/memory/sources/${encodeURIComponent(req.params.sourceId)}/entries${qs}`;
    app.handle(req, res);
  });
  app.post('/api/organizations/:orgId/memory/sources/:sourceId/entries', requireApiKey, (req, res) => {
    req.url = `/api/memory/sources/${encodeURIComponent(req.params.sourceId)}/entries`;
    app.handle(req, res);
  });
  app.post('/api/organizations/:orgId/memory/sources/github/sync', requireApiKey, (req, res) => {
    req.url = '/api/memory/sources/github/sync';
    app.handle(req, res);
  });
  app.get('/api/organizations/:orgId/memory/:memoryId', requireApiKey, orgMemoryProxy);
  app.delete('/api/organizations/:orgId/memory/:memoryId', requireApiKey, orgMemoryProxy);
  app.put('/api/organizations/:orgId/memory/:memoryId', requireApiKey, orgMemoryProxy);
}
...
function safeParse(json, fallback) {
  ...
  // Receives vault markdown files for knowledge extraction / RAG
  app.post('/api/vault/sync', requireApiKey, async (req, res) => {
    try {
    ...
    const vaultDir = openclawDataPath('vault');
      ...
      for (const f of files) {
        ...
        const safePath = f.path.replace(/\.\./g, '_').replace(/^\//, '');

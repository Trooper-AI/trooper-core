...
test('mac local installer keeps one persistent workspace binding per computer', () => {
  assert.match(script, /INSTALLATION_ID_FILE="\$TROOPER_PARENT_DIR\/install-id"/);
  assert.match(script, /EXISTING_ORG_ID="\$\(read_env_value ORG_ID\)"/);
  assert.match(script, /"\$ORG_ID" != "\$EXISTING_ORG_ID"/);
  assert.match(script, /Trooper will not replace it with workspace \$ORG_ID/);
  assert.match(script, /exit 73/);
  assert.match(script, /HOST_DEVICE_ID="\$\(tr -d '\[:space:\]' < "\$INSTALLATION_ID_FILE"\)"/);
  assert.match(script, /write_env_line TROOPER_INSTALLATION_ID "\$HOST_DEVICE_ID"/);
  assert.ok(
    script.indexOf('EXISTING_ORG_ID="$(read_env_value ORG_ID)"') < script.indexOf('docker pull "$OPENCLAW_DOCKER_IMAGE"'),
  );
});

test('mac local installer writes sourceable environment files for paths with spaces', () => {
  assert.match(script, /OPENCLAW_CONFIG_ROOT="\$\{OPENCLAW_CONFIG_ROOT:-\$OPENCLAW_DATA_DIR\/config\}"/);
  assert.match(script, /"\$OPENCLAW_CONFIG_ROOT\/devices"/);
  ...
  assert.match(script, /write_env_line OPENCLAW_GATEWAY_TOKEN "\$GATEWAY_TOKEN"/);
  assert.match(script, /write_env_line OPENCLAW_DATA_ROOT "\$OPENCLAW_DATA_DIR"/);
  assert.match(script, /write_env_line OPENCLAW_CONFIG_ROOT "\$OPENCLAW_CONFIG_ROOT"/);
  assert.match(script, /write_env_line OPENCLAW_DEVICES_DIR "\$OPENCLAW_DATA_DIR\/devices"/);
  assert.match(script, /write_env_line OPENCLAW_PAIRED_JSON_PATH "\$OPENCLAW_DATA_DIR\/devices\/paired\.json"/);
...
test('mac local installer seeds root OpenClaw gateway auth before container start', () => {
  assert.match(script, /OPENCLAW_ROOT_CONFIG_PATH="\$OPENCLAW_DATA_DIR\/openclaw\.json" GATEWAY_TOKEN="\$GATEWAY_TOKEN" node <<'NODE'/);
  assert.match(script, /config\.gateway\.auth\.token = token/);
  assert.ok(
    script.indexOf('OPENCLAW_ROOT_CONFIG_PATH="$OPENCLAW_DATA_DIR/openclaw.json"') <
      script.indexOf('cat > "$BIN_DIR/start-gateway.sh"'),
  );
});

test('mac local installer can install and start a CLI Docker runtime with Colima', () => {
  assert.match(script, /OPENCLAW_DOCKER_IMAGE="\$\{OPENCLAW_DOCKER_IMAGE:-ghcr\.io\/trooper-ai\/trooper-gateway:latest\}"/);
  assert.match(script, /OPENCLAW_GATEWAY_CONTAINER="\$\{OPENCLAW_GATEWAY_CONTAINER:-openclaw-openclaw-gateway-1\}"/);
  assert.match(script, /"\$OPENCLAW_DATA_DIR\/devices"/);
  ...
  assert.match(script, /-v "\$\{TROOPER_BRIDGE_DIR\}\/startup\.sh:\/opt\/startup\.sh:ro"/);
  assert.match(script, /--entrypoint \/bin\/bash/);
  assert.match(script, /-e "OPENCLAW_GATEWAY_TOKEN=\$\{GATEWAY_TOKEN\}"/);
  assert.match(script, /-e "TROOPER_GATEWAY_SKIP_DOCTOR=1"/);
  assert.match(script, /"\$\{OPENCLAW_DOCKER_IMAGE\}" \\\n    \/opt\/startup\.sh \\\n    "\$\{GATEWAY_PORT\}"/);
});
...
test('mac local installer retries stale git remote ref locks', () => {
  assert.match(script, /git -C "\$BRIDGE_DIR" fetch --all --prune \|\| \{/);
  assert.match(script, /rm -f "\$BRIDGE_DIR\/\.git\/refs\/remotes\/origin\/main" "\$BRIDGE_DIR\/\.git\/packed-refs\.lock"/);
  assert.match(script, /git -C "\$BRIDGE_DIR" fetch origin main --prune/);
});

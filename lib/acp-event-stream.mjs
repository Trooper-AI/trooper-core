...
/**
 * The one mapping from rule reasons to published errorKind values. Unknown
 * (custom pack) reasons return null rather than minting strings the control
 * plane's vocabulary does not know — every classification path must use this
 * so poll, push, and detail-route lanes agree for the same session state.
 */
export function acpErrorKindForRuleReason(reason) {
  return RULE_REASON_ERROR_KINDS[reason] || null;
}

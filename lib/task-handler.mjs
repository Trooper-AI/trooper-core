...
Complete this task. Use your tools. Show your work.`;
  ...
  try {
    ...
    const gatewayAgentId = agent.role === 'SPC'
      ...
      : 'main';
    const sessionKey = `agent:${gatewayAgentId}:hook:trooper:${agentSlug}:task:${taskId}`;

    // Liveness mark for the orphan-run watchdog (lib/run-watchdog.mjs).
    markRunStarted(runId, { sessionKey, taskId, agentId: agent.id, agentName: agent.name });
    ...
    try {
      result = await gateway.runAgentStreaming(
        ...
        if (stream === 'tool_result' && data) {
          ...
          // (lib/tool-result-spill.mjs). Candidate order mirrors what
          // forwardToolEventPayload would pick for `raw`; the raw override is
          // only set when a spill actually happened.
          const rawCandidate = [data.raw, data.content, data.output]
            .find((value) => typeof value === 'string' && value !== '');
          const spillOutcome = rawCandidate !== undefined
            ? maybeSpillToolResultText(rawCandidate)
            : { spilled: false };
          const toolResultPayload = forwardToolEventPayload('tool_result', data, {
            tool: toolName,
            success,
            summary,
            // When spilled, `result` is dropped along with the full raw — it is
            // the same bytes in parsed form; the spill locator carries the rest.
            ...(spillOutcome.spilled ? { raw: spillOutcome.text, result: null } : {}),
          });

...
test('resolveSlotResourceLimits prefers slot then options then env defaults', () => {
  const fromSlot = resolveSlotResourceLimits({ cpuLimit: '2', memoryLimitMb: 2048, pidsLimit: 400 }, {}, {});
  assert.equal(fromSlot.cpuLimit, '2');
  assert.equal(fromSlot.memoryLimitMb, 2048);
  assert.equal(fromSlot.pidsLimit, 400);

  const fromOpts = resolveSlotResourceLimits({}, { cpuLimit: '0.75', memoryLimitMb: 512 }, {});
  assert.equal(fromOpts.cpuLimit, '0.75');
  assert.equal(fromOpts.memory, '512m');
});

test('buildSlotGatewayRunArgs uses isolated mounts, loopback ports, and cgroup caps', () => {
  ...
  assert.equal(args[args.indexOf('--memory') + 1], '1024m');
...
test('ensureSlotRuntimeFiles repairs existing shared slot gateway config', () => {
  ...
  assert.equal(config.gateway.controlUi.dangerouslyDisableDeviceAuth, true);
  assert.equal(config.browser.headless, true);
  assert.equal(config.tools.exec.host, 'auto');

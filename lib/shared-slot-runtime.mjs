...
export async function verifySlotRuntimeReady(slot, { gatewayToken = '', bridgeAuthToken = '' } = {}) {
  ...
  if (bridgeHealth.data?.status !== 'ok' || bridgeHealth.data?.gateway_available === false || bridgeHealth.data?.gateway_state !== 'ready') {
    const reason = bridgeHealth.data?.reason || bridgeHealth.data?.gateway?.lastCloseReason || bridgeHealth.data?.gateway_state || 'not_ready';
...
export async function startSlotRuntime(slot, options = {}) {
  ...
  await waitForHealth(`http://127.0.0.1:${slot.ports.bridge}/health`, {
    timeoutMs: 90000,
    isReady: (health) => health.data?.status === 'ok'
      && health.data?.gateway_available !== false
      && health.data?.gateway_state === 'ready',
  });

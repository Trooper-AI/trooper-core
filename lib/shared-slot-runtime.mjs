...
import { ensureVolumeSkillPacks } from './default-skill-pack.mjs';
...
export const DEFAULT_GATEWAY_TRUSTED_PROXIES = Object.freeze([
  '127.0.0.1',
  '::1',
  '172.16.0.0/12',
  '173.245.48.0/20',
...
const OPENCLAW_VALID_PROVIDER_APIS = new Set([
  'openai-completions',
  'openai-responses',
...
export function buildSlotAllowedOrigins(slot = {}) {
  const gatewayPort = Number(slot.ports?.gateway || 33000);
...
/**
 * Per-slot cgroup / resource limits (density isolation P2).
 * Prefer slot-stored limits, then options, then env defaults.
 */
export function resolveSlotResourceLimits(slot = {}, options = {}, env = process.env) {
  const cpuRaw = options.cpuLimit
    ?? slot.cpuLimit
    ?? env.TROOPER_SLOT_CPU_LIMIT
    ?? '1';
  const memoryMbRaw = options.memoryLimitMb
    ?? slot.memoryLimitMb
    ?? env.TROOPER_SLOT_MEMORY_LIMIT_MB
    ?? '1536';
  const pidsRaw = options.pidsLimit
    ?? slot.pidsLimit
    ?? env.TROOPER_SLOT_PIDS_LIMIT
    ?? '512';

  const cpuLimit = String(cpuRaw || '').trim();
  const memoryLimitMb = Math.max(128, Number.parseInt(memoryMbRaw, 10) || 1536);
  const pidsLimit = Math.max(64, Number.parseInt(pidsRaw, 10) || 512);
  const memorySwap = String(env.TROOPER_SLOT_MEMORY_SWAP || `${memoryLimitMb}m`).trim();
...
export function buildSlotGatewayRunArgs(slot, {
  image = process.env.OPENCLAW_DOCKER_IMAGE || 'ghcr.io/trooper-ai/trooper-gateway:latest',
  gatewayToken = '',
  cpuLimit,
  memoryLimitMb,
  pidsLimit,
  ...
  const limits = resolveSlotResourceLimits(slot, { cpuLimit, memoryLimitMb, pidsLimit }, env);
  return [
    'run', '-d',
    '--name', containerName,
    '--restart', 'unless-stopped',
    '--user', '0:0',
    // Isolation: bind only to loopback (never publish on all interfaces).
    '-p', `127.0.0.1:${slot.ports.gateway}:${slot.ports.gateway}`,
    // cgroup caps — noisy neighbor protection on multi-tenant pool hosts.
    '--cpus', String(limits.cpuLimit),
    '--memory', limits.memory,
    '--memory-swap', limits.memorySwap,
    '--pids-limit', String(limits.pidsLimit),
    '--security-opt', 'no-new-privileges:true',
    // Per-slot volumes only — never share paths across tenants.
...
export function buildMinimalOpenClawConfig(slot, { gatewayToken = '' } = {}) {
  return {
    ...
    tools: {
      ...
      exec: { host: 'auto', mode: 'full', notifyOnExit: true },
    },
    browser: {
      enabled: true,
    ...
    gateway: {
      ...
      trustedProxies: [...DEFAULT_GATEWAY_TRUSTED_PROXIES],
      controlUi: {
        enabled: true,
        allowInsecureAuth: true,
        allowedOrigins: buildSlotAllowedOrigins(slot),
...
function sanitizeProviderApi(provider, repairs, pathPrefix, providerId = '') {
  if (!provider || typeof provider !== 'object' || Array.isArray(provider)) return;
  const api = String(provider.api || '').trim();
  if (api === 'openai-codex-responses' || (providerId === 'openai-codex' && api && !OPENCLAW_VALID_PROVIDER_APIS.has(api))) {
    provider.api = 'openai-chatgpt-responses';
    repairs.push(`${pathPrefix}.api: migrated to openai-chatgpt-responses`);
  } else if (api && !OPENCLAW_VALID_PROVIDER_APIS.has(api)) {
    delete provider.api;
    repairs.push(`${pathPrefix}.api: removed unsupported value "${api}"`);
  }
  if (Array.isArray(provider.models)) {
    for (const [index, model] of provider.models.entries()) {
      if (!model || typeof model !== 'object' || Array.isArray(model)) continue;
      const modelApi = String(model.api || '').trim();
      if (modelApi === 'openai-codex-responses' || (providerId === 'openai-codex' && modelApi && !OPENCLAW_VALID_PROVIDER_APIS.has(modelApi))) {
        model.api = 'openai-chatgpt-responses';
        repairs.push(`${pathPrefix}.models.${index}.api: migrated to openai-chatgpt-responses`);
      } else if (modelApi && !OPENCLAW_VALID_PROVIDER_APIS.has(modelApi)) {
        delete model.api;
        repairs.push(`${pathPrefix}.models.${index}.api: removed unsupported value "${modelApi}"`);
      }
    }
  }
}

export function sanitizeSlotOpenClawConfig(config, slot, { gatewayToken = '' } = {}) {
  const repairs = [];
  ...
  if (next.meta !== undefined) {
    delete next.meta;
    repairs.push('meta: removed unsupported Trooper metadata');
  }

  if (!next.models || typeof next.models !== 'object' || Array.isArray(next.models)) {
    next.models = { providers: {} };
    repairs.push('models: initialized');
  }
  if (!next.models.providers || typeof next.models.providers !== 'object' || Array.isArray(next.models.providers)) {
    next.models.providers = {};
    repairs.push('models.providers: initialized');
  }
  if (Object.prototype.hasOwnProperty.call(next.models.providers, 'composio')) {
    delete next.models.providers.composio;
    repairs.push('models.providers.composio: removed non-model provider overlay');
  }
  for (const [providerId, provider] of Object.entries(next.models.providers)) {
    sanitizeProviderApi(provider, repairs, `models.providers.${providerId}`, providerId);
  }
  ...
  if (model && typeof model === 'object' && !Array.isArray(model)) {
    ...
    if (primary.startsWith('openai-codex/') && !next.models.providers['openai-codex']) {
      model.primary = desired.agents.defaults.model.primary;
      repairs.push('agents.defaults.model.primary: replaced unsupported openai-codex model');
    }
    if (Array.isArray(model.fallbacks)) {
      const fallbacks = model.fallbacks.filter((value) => {
        const text = String(value || '').trim();
        return text && !(text.startsWith('openai-codex/') && !next.models.providers['openai-codex']);
      });
      if (fallbacks.length !== model.fallbacks.length) {
        model.fallbacks = fallbacks;
        repairs.push('agents.defaults.model.fallbacks: removed unsupported openai-codex fallback');
      }
    }
  }
...
export function ensureSlotOpenClawConfig(configPath, slot, { gatewayToken = '' } = {}) {
  ...
  config.gateway = {
    ...
    controlUi: {
      ...
      allowedOrigins: uniqueStrings([
        ...(Array.isArray(existingControlUi.allowedOrigins) ? existingControlUi.allowedOrigins : []),
        ...desired.gateway.controlUi.allowedOrigins,
      ]),
    },
    http: {
  ...
  const { config: sanitizedConfig } = sanitizeSlotOpenClawConfig(config, slot, { gatewayToken });
  return writeJsonIfChanged(configPath, sanitizedConfig);
}
...
export function ensureSlotRuntimeFiles(slot, { gatewayToken = '' } = {}) {
  ...
  const workspaceDirs = ['Channels', 'Tasks', 'apps', 'memory', 'skills'];

  for (const dirName of workspaceDirs) {
    fs.mkdirSync(path.join(slot.paths.workspaceRoot, dirName), { recursive: true });
  }

  const configChanged = ensureSlotOpenClawConfig(configPath, slot, { gatewayToken });
  const authProfilesChanged = writeJsonIfMissing(authProfilesPath, { version: 1, profiles: {}, lastGood: {}, usageStats: {} });
  ...
  // Density P5: default skill pack is volume files on the slot, not host packages.
  let skillPacks = [];
  try {
    skillPacks = ensureVolumeSkillPacks({
      workspaceRoot: slot.paths?.workspaceRoot,
      configRoot: slot.paths?.configRoot,
    });
  } catch (error) {
    console.warn(`[shared-slot] skill pack ensure failed for ${slot.slotId}: ${error.message}`);
  }
...
async function waitForHealth(url, { timeoutMs = 90000, pollMs = 1500, onFailure = null, isReady = null } = {}) {
  const started = Date.now();
  let last = null;
  while (Date.now() - started < timeoutMs) {
    last = await fetchJsonHealth(url, 3000);
    if (last.ok && (!isReady || isReady(last))) return last;
...
export async function startGatewayContainer(slot, {
  gatewayToken,
  image,
  forceRestart = false,
  cpuLimit,
  ...
  const args = buildSlotGatewayRunArgs(slot, {
    image,
    gatewayToken,
    cpuLimit: cpuLimit ?? slot.cpuLimit,
    memoryLimitMb: memoryLimitMb ?? slot.memoryLimitMb,
    pidsLimit: pidsLimit ?? slot.pidsLimit,
  });
  await new Promise((resolve, reject) => {
...
export async function verifySlotRuntimeReady(slot, { gatewayToken = '', bridgeAuthToken = '' } = {}) {
  const configPath = path.join(slot.paths.configRoot, 'openclaw.json');
  ...
  const gwConnected = Boolean(bridgeHealth.data?.gateway?.websocketConnected || bridgeHealth.data?.gateway?.connected || bridgeHealth.data?.mode === 'websocket');
  const gwReady = bridgeHealth.data?.status === 'ok'
    || bridgeHealth.data?.gateway_state === 'ready'
    || bridgeHealth.data?.gateway?.gateway_state === 'ready'
    || gwConnected;
  if (bridgeHealth.data?.gateway_available === false || !gwReady) {
    const reason = bridgeHealth.data?.reason || bridgeHealth.data?.gateway?.lastCloseReason || bridgeHealth.data?.gateway_state || bridgeHealth.data?.status || 'not_ready';
    throw new Error(`slot bridge gateway not ready: ${reason}`);
  }
  ...
  try {
    filesResponse = await fetch(
      `http://127.0.0.1:${slot.ports.bridge}/files?path=${encodeURIComponent('/home/node/.openclaw/workspace')}`,
      {
        headers: bridgeAuthToken ? { Authorization: `Bearer ${bridgeAuthToken}` } : {},
        signal: AbortSignal.timeout(5000),
      },
    );
    filesData = await filesResponse.json().catch(() => ({}));
  } catch (error) {
    throw new Error(`slot bridge files check failed: ${formatFetchError(error)}`);
  }
  if (!filesResponse.ok) {
    throw new Error(`slot bridge files check failed: HTTP ${filesResponse.status} ${String(filesData?.error || '').slice(0, 180)}`.trim());
  }
  if (!Array.isArray(filesData?.files)) {
    throw new Error('slot bridge files check failed: organized files array missing');
  }
  ...
  return {
    ...
    files: { status: filesResponse.status, count: filesData.files.length },
  };
}
...
export async function startBridgeProcess(slot, {
  ...
  if (forceRestart && fs.existsSync(pidPath)) {
  ...
  if (health.ok && !forceRestart) return { pid: null, reused: true };
...
export async function startSlotRuntime(slot, options = {}) {
  ...
  const gateway = await startGatewayContainer(slot, {
    gatewayToken,
    image: options.image,
    forceRestart: files.configChanged === true || options.forceGatewayRestart === true,
    cpuLimit: options.cpuLimit ?? slot.cpuLimit,
    memoryLimitMb: options.memoryLimitMb ?? slot.memoryLimitMb,
    pidsLimit: options.pidsLimit ?? slot.pidsLimit,
  });
  const bridge = await startBridgeProcess(slot, {
    ...
    forceRestart: files.configChanged === true || options.forceBridgeRestart === true,
  });
  await waitForHealth(`http://127.0.0.1:${slot.ports.bridge}/health`, {
    timeoutMs: 120000,
    isReady: (health) => {
      const d = health.data || {};
      if (d.gateway_available === false) return false;
      if (d.status === 'ok' && (d.gateway_state === 'ready' || d.gateway?.gateway_state === 'ready')) return true;
      if (d.gateway?.websocketConnected || d.gateway?.connected || d.mode === 'websocket') return true;
      return false;
    },
  });
  const verification = await verifySlotRuntimeReady(slot, { gatewayToken, bridgeAuthToken });

...
async function waitForHealth(url, { timeoutMs = 90000, pollMs = 1500 } = {}) {
  ...
  throw new Error(last?.error || `health check timed out for ${url}`);
}

export async function startGatewayContainer(slot, { gatewayToken, image } = {}) {
  const containerName = buildSlotContainerName(slot.slotId);
  const health = await fetchJsonHealth(`http://127.0.0.1:${slot.ports.gateway}/healthz`, 1500);
  ...
  await waitForHealth(`http://127.0.0.1:${slot.ports.gateway}/healthz`, { timeoutMs: 90000 });

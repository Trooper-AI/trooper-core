...
export { normalizeMessageOutput } from './message-json-fields.mjs';
...
export function normalizeMessageInput(body = {}) {
  ...
  for (const [key, value] of Object.entries(body || {})) {
    ...
    if (!column) continue;
    if (MESSAGE_JSON_FIELDS.has(column)) {
      row[column] = serializeMessageJsonField(value);
      continue;
    }
    if (column === 'fallback') {
...
export function getMessage(id) {
  ...
  return normalizeMessageOutput(
    db.select().from(messagesTable).where(eq(messagesTable.id, id)).get() || null,
  );
}
...
export function listMessages({ channel, limit = 50 } = {}) {
  ...
  return results.reverse().map(normalizeMessageOutput);
}

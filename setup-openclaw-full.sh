...
if [ -z "$TROOPER_EMAIL_ENABLED" ] || echo "$TROOPER_EMAIL_ENABLED" | grep -q '{{.*}}'; then
  TROOPER_EMAIL_ENABLED="0"
fi
if [ "$TROOPER_EMAIL_ENABLED" != "0" ] && [ "$TROOPER_EMAIL_ENABLED" != "1" ]; then
  echo "FATAL: TROOPER_EMAIL_ENABLED must be 0 or 1"
  exit 1
fi
...
_validate_var TROOPER_EMAIL_ENABLED "$TROOPER_EMAIL_ENABLED" optional
...
OPENCLAW_COMPANY_PROVIDER_KEYS=${OPENCLAW_COMPANY_PROVIDER_KEYS}
...
Environment=OPENCLAW_BRIDGE_DEVICE_AUTH=1
...
Environment=TROOPER_EMAIL_ENABLED=${TROOPER_EMAIL_ENABLED}
